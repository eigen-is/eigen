...
| **CalDAV**            | `apps/api/src/lib/caldav/`                   | RFC 4791 at `/dav/calendars/:ownerId/…`, the protocol layer only. The iCalendar parser, component builder and VTIMEZONE generator every `.ics` surface shares — the store, iMIP, import, quick look — are in `apps/api/src/lib/ical/`, the `lib/vcard/` twin. See [CALENDAR.md](CALENDAR.md) |
