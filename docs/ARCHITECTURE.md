...
| **Indexed file store** | `apps/api/src/lib/core/indexed-file-store.ts` | SQL-free files-as-truth core shared by contacts and calendar: the `WriteGate`, the file/key helpers, the stat diff, the rebuild generation, the uid guard, the broadcast batch, and `displaceUnindexedFile` — the rule that a create moves a file no index row claims aside instead of overwriting it. See [CONTACTS.md](CONTACTS.md) and [CALENDAR.md](CALENDAR.md) |
