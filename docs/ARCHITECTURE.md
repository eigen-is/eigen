...
| **Blob store**        | `apps/api/src/lib/core/blob-store.ts`        | The SQL-free half both DAV domains share, over the rule that the stored blob is the truth and every other column a projection of it: the uri normalization (`normalizeResourceUri`, NFC only) and the name rule over it, the content-hash etag, the generation seed `newSyncGen`, the cold size reader `readBlobTableSize` (0 for a database whose blob migration has not run yet), the `ResourcePreconditions` both write paths evaluate, the broadcast batch, and the store result types. See [CONTACTS.md](CONTACTS.md) and [CALENDAR.md](CALENDAR.md) |
| **Instance lock**     | `apps/api/src/instance-lock.ts`              | One API process per data dir: `index.ts` takes it, then loads `server.ts`, before anything opens a database. See [DATABASE.md § Instance lock](DATABASE.md#instance-lock) |
...
| **CardDAV**           | `apps/api/src/lib/carddav/`                  | RFC 6352 at `/dav/addressbooks/:ownerId/…`; a card's vCard bytes are the truth and live in the `vcard` BLOB of its `contacts.db` row, which also carries the projected columns and the sync/label metadata no card holds. The vCard parser/serializer/transcoder is backend-only, in `apps/api/src/lib/vcard/`, and `apps/api/src/lib/contacts/transfer.ts` replays a whole `.vcf` file through the same PUT seam. See [CONTACTS.md](CONTACTS.md) |
| **CalDAV**            | `apps/api/src/lib/caldav/`                   | RFC 4791 at `/dav/calendars/:ownerId/…`, the protocol layer only; a resource's VCALENDAR bytes are the truth and live in the `ics` BLOB of its `calendar.db` row, beside the projected `events` rows and the calendar metadata no resource holds (name, color, shares, `ctag`, `syncGen`). The iCalendar parser, component builder and VTIMEZONE generator every `.ics` surface shares — the store, iMIP, import, export, quick look — are in `apps/api/src/lib/ical/`, the `lib/vcard/` twin, and `apps/api/src/lib/calendar/transfer.ts` replays a whole `.ics` file through the same PUT seam. See [CALENDAR.md](CALENDAR.md) |
| **Shared DAV core**   | `apps/api/src/lib/dav/`                      | What CalDAV and CardDAV answer identically: the multistatus/propstat XML envelope and principal props (`xml.ts`), the PROPFIND prop selection (`propfind.ts`), the store-result → HTTP mapping both write surfaces take (`write-result.ts`), the sync-token grammar and its `valid-sync-token` refusal (`sync-token.ts`), the href shapes plus the multiget resolver (`href.ts`), and the byte-budgeted resource row every REPORT of both protocols emits (`report-row.ts`) |
...
| **Setup token**       | `apps/api/src/lib/setup/setup-token.ts`      | The one-time token in the setup link: only its sha256 in `data/server/setup-token.json`. `/setup/s3check`, `/setup/s3harden` and `/setup/complete` refuse without it (403, before any S3 call or write); the control socket's `POST /setup-link` (what `./eigen setup` ends with) replaces it, a development boot logs one, completing setup deletes it |
...
| **Import from Drive** | `apps/api/src/lib/drive/import-source.ts`    | `readImportSourceBytes(user, source: DriveImportSource, { accepts, rejection, maxBytes })` — the one reader behind every `import-from-drive` route (mail, contacts, calendar): resolves the source through `getSharedDrive` (404 missing), 400s a folder/Eigen container or a type the caller's predicate rejects, 413s the row size and again on the bytes, which are read under a running cap rather than an unbounded `arrayBuffer()`. What each route hands it — the rejection string, the ceiling — and the `decodeUtf8Strict` every one of them decodes with live in `lib/core/transfer.ts`, the backend's own whole-file-transfer facts |
...
| **Import to Calendar** | `packages/ui/src/components/calendar/import-to-calendar-picker.tsx` | `ImportToCalendarPicker` — the target an `.ics` import needs and the other two do not: the viewer's own calendars, the team calendars they may write in, or a new one named after the file, then the import itself. See [CALENDAR.md](CALENDAR.md) |
