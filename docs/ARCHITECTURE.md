...
| **Blob store**        | `apps/api/src/lib/core/blob-store.ts`        | The SQL-free half both DAV domains share, over the rule that the stored blob is the truth and every other column a projection of it: the uri normalization (`normalizeResourceUri`, NFC only) and the name rule over it, the content-hash etag, the rebuild generation `nextSyncGen`, the broadcast batch, and the store result types. See [CONTACTS.md](CONTACTS.md) and [CALENDAR.md](CALENDAR.md) |
