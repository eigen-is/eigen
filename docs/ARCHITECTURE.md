...
| **File subject**   | `packages/lib/src/types/file-subject.ts` + `packages/lib/src/core/file-subject.ts` | `FileSubject` — one file a surface can act on, whatever holds it (a Drive item, a mail part); `subjectFromPath()` and `subjectFromMailAttachment()` are its only builders and `getPreviewMode()` picks the overlay's render mode. See [PREVIEWS.md](PREVIEWS.md) |
...
| **File action runner** | `packages/ui/src/components/file-actions/use-file-action-runner.tsx` | `useFileActionRunner(subject, siblings?, options?)` performs a registry row and hands its host one `dialogs` node to mount, so a picker outlives the menu that opened it |
