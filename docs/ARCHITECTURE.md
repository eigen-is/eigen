...
| **Indexed file store** | `apps/api/src/lib/core/indexed-file-store.ts` | The domain-neutral half of a files-as-truth store (contacts today): the `WriteGate` that serializes every mutation and re-indexes a file whose commit threw, the file/key helpers (`sanitizeResourceUri`, `uriKeyOf`, `computeResourceEtag`, `writeResourceFile`, `readResourceFile`, `cleanupTempFiles`, `listResourceUris`), the pure stat diff `diffFileStats` and the uid guard `dedupeByUid`. It owns no SQL — the domain keeps its tables and hands in rows and callbacks. See [CONTACTS.md](CONTACTS.md) |
