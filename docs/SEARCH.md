...
**Extraction.** `apps/api/src/lib/search/extract-text.ts` dispatches on the container type first and on the mime only for a real container, since `mimeType` is caller-controlled on upload. The four collab types (doc, sheets, slides, vector) extract inside the one-shot document-transform Worker — the background `extract-text` op runs `lib/search/extract-render.ts` over the same `*FromDoc` readers of the [document content layer](DOCUMENT-CONTENT-LAYER.md) that preview and export use, so search's idea of "document text" cannot drift from them, and a heavy extraction never blocks the event loop (own 30 s deadline, background queue quota — see [DOCUMENT-TRANSFORMS.md](DOCUMENT-TRANSFORMS.md)). Thin collectors flatten each shape (ProseMirror JSON → text, the **sparse** `celldata` display values), cut to the `CONTENT_INDEX_MAX_BYTES` UTF-8 byte budget (~100 KB) at a code-point boundary. A deck and a drawing are the same document, so **one** collector serves both: `collectCanvasText` walks each element kind's own `searchText` in reading order — frame by frame, then z-order inside a frame — which is `searchScene`'s rule, so the index and the in-document find bar cannot disagree about what a canvas says. Stickies and chat stay light main-thread reads (`readStickiesContent`, `readChatContent` — card and column text; chat is relational, not Yjs, and takes the **newest** ~100 KB by paging backwards on `createdAt`). Plain files are eligible via the canonical `isSearchableTextFile` and read through `mount.readRange` so a huge file is never slurped. A `.vcf` is on that list too but takes its own branch before the raw read: its body is mostly base64 photo and a folded name is not there to be matched, so `extractVCardText` runs the background vCard preview job ([PREVIEWS.md](PREVIEWS.md)) and indexes each card's display name, emails, company and job title, bounded by the same byte budget. It inherits that job's ceilings — a file over `VCARD_MAX_BYTES` and a file the decoder refuses index as nothing rather than staying dirty for every later drain, and the cards past `VCARD_PREVIEW_MAX_CARDS` are not indexed. It runs its own job rather than reading through `getVCardPreview`, so an uploaded `.vcf` has its cards built twice (once on the drain, once on first view): the preview cache is version-keyed and answers stale-while-revalidate, and an index built from a previous version's cards would be wrong in a way nothing corrects. The same predicate drives the `searchable` column in `mount.ts` and the backfill in `db-config.ts`, so a `.vcf` is queued for extraction on upload like any other file. An `.eml` is **not** on that list under any of its mimes: its raw body is headers, boundaries and base64, and what a reader sees of it is the payload the quick look builds ([PREVIEWS.md](PREVIEWS.md)). The preview generators and the export pipeline are otherwise avoided on purpose: the first emits capped HTML, the second embeds media and can densify a sparse sheet.
