...
- [ ] `eigen`, the one command an operator learns: `eigen setup | update | rollback | backup | restore <archive> | reset-password <email> | status | logs` (`rollback` lands with block 2). Two layers, so the host needs nothing but Docker and Compose:
  ...
  - Online commands (`status`, `reset-password`) run through `docker compose exec eigen-api` and talk to the running API over a Unix socket in the container's `/run/eigen/`, mode 0600, owned by the API user: no token, no TCP. Offline commands (the first `setup`, `restore` onto a stopped stack) run through `docker compose run --rm` and write as `1000:1000`
- [ ] `eigen setup`: preflight, network snapshot, configurator in the API image, `data/` and `backups/` ownership, start and wait for healthy (the proposal's § 1). the configurator is `apps/api/src/cli/configure.ts`, and `bun run setup` runs it
...
- [ ] `eigen reset-password <email>`, a way back in for a locked-out admin; it also revokes the user's sessions and app passwords. There is no self-service reset by email, and the Users admin page needs a second admin
