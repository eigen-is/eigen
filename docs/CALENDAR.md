...
`CALENDAR_DB_CONFIG` (`db-config.ts`) is at `currentVersion: 2`. v2 carries `calendars` and `shared_calendars` over — the only rows no resource's bytes hold, and the ones other Homes point at through a share grant — reshaping each in place through an `INSERT … SELECT` that re-seeds every carried `syncGen` from the clock, so every sync token minted before it is refused. It migrates a v1 database, the only shape ever deployed: v1's `events` and `event_tombstones` are dropped, and the blob shape — `resources`, `events` and `resource_tombstones` — is created fresh. The config also sets `synchronous: 'FULL'` ([DATABASE.md § Pragmas](DATABASE.md#pragmas)): the resources themselves live in these rows, so an acknowledged CalDAV PUT must survive a power loss.
...
`rebuildProjection` (`calendar.ts`) is the tool and the proof: it parses every resource's blob and rewrites `uid`, `etag`, `hasUnindexedRecurrence` and every `events` row from it in one transaction, leaving the ctags, the generation, the tombstones and the share grants exactly as they were. Its callers today are tests — the contract pin in `test/calendar/resource-store.test.ts`, and `calendar-inbound-request.test.ts`, which plants foreign bytes in a row and rebuilds the projection from them; the next projected column is added by altering the table and calling it.
...
**One write function, and it owns the order.** Every resource path goes through `Calendar.writeResource` (`calendar/calendar.ts`), the `writeCard` twin: it takes a `PreparedResource` — the serialized text, its bytes, their hash and the rows they project to, all computed once by `prepareResource` (`resource-store.ts`, beside `prepareCard`'s twin) — so the bytes a refusal judged are the bytes that land. It holds both ceilings right before the commit: `EVENT_MAX_BYTES` on the bytes that would land, then `enforceHomeDataQuota` when the Home is metered, crediting the resource this one replaces. An event mutation reaches it through `events.writeComponent`, which prepares the component it just rewrote; a CalDAV PUT prepares first itself, because it has preconditions to evaluate against the prepared rows before it commits to writing them.

`commitResource` (`calendar.ts`, private to the facade) is the single index-write seam and one transaction: the calendar `ctag` bump, the `resources` upsert with its blob, a delete-and-reinsert of every `events` row of that resource, and the tombstone clear by `uri` (`indexResource`, `resource-store.ts`). The byte delta is read *inside* that transaction and applied to `eventsBytes` after it returns, because a rollback would otherwise leave the delta applied — the counter rule both DAV domains follow. `purgeResource` is the same shape in reverse: one transaction bumps the ctag, deletes the `resources` row (its `events` rows cascade) and writes the tombstone. A throw anywhere leaves the previous bytes readable through `getResource` and the projection exactly as it was; there is no second store to fall out of step with the first.

**Init** seeds `eventsBytes` from `SUM(length(ics))` and creates the default calendar when the Home has none, with a clock-seeded `syncGen` (`newSyncGen()`, `lib/core/blob-store.ts`: the wall clock in seconds). Metering (`meteredIngest`) is switched on at the end, because the quota lookup goes through `getHome`.
...
- `RECURRENCE-ID` names the ORIGINAL occurrence, computed from the master via `computeOccurrenceTimes`, in the master's DTSTART form, value type included — never the exception's moved `startTime`, and never `VALUE=DATE` because the override was toggled to all day (or a DATE-TIME because an all-day series' override was timed): either orphans the override (RFC 5545 §3.8.4.4).
...
- A valid IANA TZID resolves through `Intl` whether or not the file defines a VTIMEZONE — the path the builder computes its wall times with — so identical bytes name one instant and a repeated hour resolves to its first pass. A UTC `Z` value is exact; only a TZID `propTzid` cannot name (neither `Intl`, the Windows table nor its VTIMEZONE's `X-LIC-LOCATION`) resolves through the file's own VTIMEZONE; a genuinely floating datetime maps its wall components via `Date.UTC`, never through the server's zone.
...
- **The web app sends no etag on an event update.** The REST PUT takes an optional `etag` and answers 412 when it does not match the resource, like the contacts twin, but `useUpdateEvent` sends none, so two web clients editing one event last-write-win. Wiring it is mostly single PUTs; the one flow that chains writes to one resource is the edit dialog's update→move. Delete, move and the creation of an occurrence override take no token at all ([ROADMAP.md](ROADMAP.md)).
...
- **An override that names no guests reaches an external guest without them.** Eigen reads such an override as inheriting the series' list, but its iMIP VEVENT and its stored VEVENT carry no guest `ATTENDEE` line ([ROADMAP.md](ROADMAP.md)).
...
- **`apps/api/src/lib/calendar/`** — the domain, split Mount-style: `calendar.ts` (the `Calendar` facade — the write lock, the database handle, the commit seams, the byte counter and the announcements; every sibling call goes through it) with sibling modules of plain functions over it: `dav-store.ts` (the CalDAV store seam — the row reads and `putResource`/`deleteResource`, which prepare and then call the facade's one write), `events.ts` (create, update, delete and move, plus the locked internals every other sibling writes an event through), `invitations.ts` (every inbound scheduling message and every RSVP), `shares.ts`, `occurrences.ts` (the range reads), `transfer.ts` (whole-file export and import), `get-calendar.ts` (access resolution, the Drive `get-drive.ts` analogue). Beside them: `resource-store.ts` (what a resource means as columns — `sanitizeCalendarId`, `sanitizeEventUri`, `resourceBytes`, `readCalendarTotalSize`, `projectRows`, `prepareResource`, `indexResource`, `PURGED_RESOURCE`, `EVENT_MAX_BYTES` — over the domain-neutral half both DAV domains share in `lib/core/blob-store.ts`: `normalizeResourceUri`, `sanitizeResourceUri`, `computeResourceEtag`, the generation seed `newSyncGen`, the cold size reader `readBlobTableSize`, the shared `ResourcePreconditions`, the `BroadcastBatch` and the store result types, none of which knows the schema), `share-propagation.ts`, `invite-propagation.ts`, `imip.ts`, `recurrence.ts`, `event-input.ts`, `mappers.ts`, `types.ts` (the inbound payload shapes), `schema.ts`, `db-config.ts`, `sse-events.ts`.
