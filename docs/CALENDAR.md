...
Regression nets: `caldav.test.ts` (protocol), `caldav-roundtrip.test.ts` (serialization/parse round-trips, TZ-pinned floating tests), `vtimezone.test.ts` (generator vs `Intl`), `calendar-timezone.test.ts` (occurrence keying), `ical-imip.test.ts` (iMIP scoping), `caldav-client-sync.test.ts` (client-faithful sync flows), `ical-parse.test.ts` (multi-series files), `ical-component.test.ts` (kitchen-sink fidelity under a patch, the SEQUENCE rule, stamp trust).
...
1. **Update.** A stored event already linked to an organizer wins, *provided* the sender is the organizer that copy is linked to — a co-attendee cannot hijack an invitation. A REQUEST carrying a `RECURRENCE-ID` attaches as an exception; a full update would collapse the series.
...
The transfer hooks live beside them in `use-transfer.ts`: `useImportCalendar` takes a `FileImportSource` plus the target `calendarId` in its mutation variables and reports the three counts in one toast.
