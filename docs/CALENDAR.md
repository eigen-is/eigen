...
3. **Create**, in the default calendar, but only when the body's `ORGANIZER` *is* the sender: a new invitation is attributed to its sender, so the two must agree. A REQUEST naming one occurrence of a series this Home does not hold files as a **standalone event** that keeps its `RECURRENCE-ID` — the only durable record of which occurrence it answers for, and what a later update or cancellation of that occurrence finds; an invitation to the whole series then replaces it. A Home with no default calendar is dropped.
