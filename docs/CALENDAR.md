...
- **The web app sends no etag on an event update.** The REST PUT takes an optional `etag` and answers 412 when it no longer matches the resource, like the contacts twin, but the dialogs don't send one yet because they chain several writes to one resource, so two web clients editing one event last-write-win ([ROADMAP.md](ROADMAP.md)).
