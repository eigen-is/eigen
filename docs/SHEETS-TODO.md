...
- [ ] **Seven document-wide element lookups assume one workbook per page** — `hyperlink.ts:173,191` (the link range-selection modal, matched by a class pair), `image.ts:102,129` (`#sheet-modal-dialog-activeImage`), `formula-range.ts:124,128` (`.sheet-formula-text-lpar`) and `data-verification.ts:603` (`#sheet-dataVerification-dropdown-btn`) all query `document` and take the first hit; both ids are rendered inside the workbook subtree, so two mounts emit duplicate ids. `image.ts` has the same MoveStart/Move pair shape as the link modal — dragging an image in workbook B would move A's. The same mount would also duplicate the per-workbook ids nothing queries but assistive tech does — `sr-selection`, `sr-sheetFocus`, `shortcuts-heading` (`SheetOverlay/index.tsx`). **Latent today**: `apps/sheets/src/components/sheets/editor.tsx:343` is the only `<Workbook>` mount in the repo, so nothing can hit this until a second one exists (a side-by-side compare view, a preview pane). The keyboard half was fixed on 2026-08-28 only because the handler was already handed its own `cellInput`. The blocker the old note claimed is not real: `refs.globalCache` is per-workbook (`Workbook/index.tsx:149`) and is already the argument these handlers receive, so one container field set at mount closes all seven from one place. **M**

- [ ] **The cell-range picker is not on the shared dialog, and looks it** — `LinkEditCard` renders the link editor as a proper shadcn `Dialog` (`components/LinkEditCard/index.tsx:232`) but the range picker as a hand-rolled card (`:170-225`): a raw `text-base font-medium` heading where `DialogHeader`/`DialogTitle` belong, buttons in a bare flex row instead of `DialogFooter`, an absolutely-placed `X` at 22px, and no gap scale. Screenshot 2026-08-28 shows the result — oversized title, cramped footer. It is hand-rolled for a reason: a modal dialog would block the cell clicks the picker exists to collect. Radix supports that — the shared `Dialog` forwards props to `DialogPrimitive.Root`, so `<Dialog modal={false}>` works — but `DialogContent` always renders `DialogOverlay` (`packages/ui/src/components/dialog.tsx:63`), a `fixed inset-0` blocker, so the shared primitive needs a way to omit it. Decide that shape first: a `showOverlay={false}` prop on `DialogContent` (touches a primitive every app uses) versus reusing `DialogHeader`/`DialogFooter` on the existing card for the look alone. Also settles two other entries: the picker's drag-to-move is broken today (mousedown → `onRangeSelectionModalMoveStart` is wired, but the card doesn't move — reported 2026-08-28, mechanism not yet found), and a real dialog would delete the `hyperlink.ts` half of the document-wide-lookup bug outright. **M**
