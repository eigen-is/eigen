...
- [ ] **Undoing a column or row resize doesn't always work** — reported 2026-08-28. Suspected mechanism, not yet proven: the resize mouseup takes `const cfg = ctx.config` (`state/events/mouse-drag.ts:467,534`), mutates it, then assigns the *same object* to both `ctx.config` and `ctx.sheets[idx].config` (`:582-586`). Immer generates patches against a state tree that now aliases one object at two paths, so an inverse patch can restore one and not the other — leaving the persisted config reverted while the geometry the renderer reads (`ctx.config`) is not, which shows up as "undo did nothing". Same aliasing family as the paste row-height bug fixed on 2026-08-28. Start with a failing test: resize a column, undo, assert `ctx.config.columnlen` **and** `sheets[idx].config.columnlen` both revert. **M**
