...
Verified, user-visible, bounded. Each remaining entry needs a decision first — three carry a small design choice, one needs its fix shape chosen — unlike the seven fixed on 2026-08-28.
...
- [ ] **The selection's drag-to-move and fill targets are too small to hit** — reported 2026-08-28 from the running app. **The previous entry here was wrong and is retracted**: drag-to-move is NOT unreachable and `move-cells.ts` is NOT dead code. Four edge strips are rendered — `.sheet-cs-draghandle-top/-bottom/-left/-right` (`OverlayVisuals.tsx:177-192`) — with `cursor: move` and `pointer-events: auto`, and their `onMouseDown` only `preventDefault()`s, so the event bubbles to `.sheet-cell-selected`'s handler and fires `onCellsMoveStart`. The parent's `pointer-events: none` is irrelevant. The old entry asserted the fill handle was the only `pointer-events: auto` child; it is not. **Do not delete `move-cells.ts`.** The real defect is hit-target size: the strips are `2px` plus a 2px transparent border at a `-4px` offset (~6px of band), so the move only starts when the pointer is exactly on the border, and the fill handle is `6×6px` at `bottom/right: -5px`. Both are invisible chrome (`opacity: 0.01` on the strips), so the hit area can grow without changing the look. Watch the trade: a click just inside the selection must still start a new selection. **S**
...
- [ ] **Growing a cell's font doesn't grow its row** — `updateFormatCell` (`toolbar.ts:148-165`) takes `cfg = ctx.sheets[sheetIndex].config` for the `attr === 'fs'` auto-fit and writes `rowlen` sheet-only. Same mirror class as the three fixed in `37923bf98` — the height is stored and syncs, the grid keeps painting the old one until a reseed. Untested. **S**

- [ ] **A wrapped cell only auto-grows if it holds multi-line text** — independent of the mirror. `cell.ts` calls `getCellTextInfo` with `{r, c, cellWidth}` and no `cellHeight`, so `cellHeight` defaults to `0` and the wrap-split loop breaks at its first `parsedTextHeight >= cellHeight` check. Measured: a 43-char string in a 40px column reports `textHeightAll = 11` — one line — so a plain long string in a narrow wrapped column never grows its row. Only the inline-string path fires today. `toolbar.ts:154` calls it the same way. **M**

- [ ] **Data validation is not carried by the fill handle** — `updateDropCell` clones `file.dataVerification` (`drop-cell.ts:~1902`), mutates it (`~2038`) and never writes it back, so dragging a cell with a validation rule carries nothing. Twin of the border carry-over dead write fixed in `37923bf98`; the fix is one line (write `file.dataVerification` directly — source and apply ranges are disjoint). Held back deliberately: this turns on behaviour that has been off for the fork's lifetime, which is a product call, not a broken window. **S**
...
A manual pass over the DOM chrome after the `sheet-` rename. Recorded so it isn't re-tested: freeze panes (line, handles, frozen scrolling), row/column resize itself, the fill handle, the data-validation dropdown (already a shadcn `DropdownMenu` — `components/DataVerification/DropdownList.tsx:53`), and the link editor (a shadcn `Dialog`, centred and deliberately not draggable). The fill handle extends along one axis at a time. This was recorded as matching Excel and Google Sheets and therefore not a defect — **unverified, and doubted on 2026-08-28**: both are believed to fill a rectangle when dragged diagonally. Check against the real products before relying on this line. `Ctrl`+arrow does nothing **on macOS** because Mission Control takes all four before the browser sees them; `Cmd`+arrow is the Mac binding and is handled (`handleControlPlusArrowKey`), with `preventDefault` so the browser's back/forward doesn't fire.
...
- [ ] **Dead upstream chrome CSS** — `.sheet-postil-*` (17 rules, `SheetOverlay/index.css`) styles the fortune-sheet "postil" comment dialog that SHEETS.md records as fully removed in favour of Eigen comment cards; `.sheet-cs-touchhandle-lb` and `-rt` are dead the same way (`OverlayVisuals.tsx:193,196` renders only `-lt` and `-rb`). No producer for any of them; confirmed unmatched in a live editor session on 2026-08-28. **S**
...
| Fill handle carries no borders | **Fixed** in `37923bf98`. `updateDropCell` pushed carried border entries into a `cloneDeep(ctx.config)` and never wrote the clone back to either half — a pure dead write, so the fill painted no border at all. Its `dataVerification` twin is still dead, filed above |
| Undoing a column or row resize doesn't always work | **Fixed** in `e74cb5c54`. **The recorded aliasing hypothesis was wrong** — immer emits a correct, complete inverse pair over an aliased object; applying it unfiltered restores both paths. Two real causes. (1) `ctx.config` is a derived mirror of the current sheet's config and is what every geometry read goes through (`calcRowColSize`, and the recompute effect keyed on `context.config.columnlen`), but `filterPatch` keeps only `sheets[*]` patches, so undo reverted the sheet and left the mirror — then a later reseed snapped the old size back, which is the "doesn't *always*" part. `updateContextWithSheetConfig` re-points the mirror; `handleUndo`, `handleRedo` and `applyOp` call it after applying patches. `applyOp` was the same bug for a **peer's** resize, which never appeared at all. (2) `setRowHeight`/`setColumnWidth` mutated the sheet's config and then re-pointed the mirror at it; assigning a draft into a slot whose base holds a different value collapses immer's patches into one top-level `['config']` patch, which `filterPatch` drops **outright** — that path emitted no undo entry and no op at all, so the old `TODO(sizing-sync)` note was true only there. They now write mirror-first like the drag handlers. Do not widen `filterPatch`: a top-level `['config']` op carries no sheet id |
