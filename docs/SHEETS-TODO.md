...
- The `sheet-copy-action-table` marker string is load-bearing in three modules and a committed test fixture. Renamed from `fortune-copy-action-table` on 2026-08-28; the three read sites match the `copy-action-table` **suffix**, so a clipboard written by either build is recognised. Keep that suffix stable — matching the full string again would make a pre-rename clipboard read as an eigen text item and paste as tab-separated plain text, which is worse than the foreign-HTML fallback.
...
- [ ] **The HTML class-block parser is a fixture-shaped heuristic** — `paste.ts:1386-1395` pairs two independent regex scans **by index** (`zip(patternName, patternStyle)`), with `initial()` compensating for the MSO comment's `-->` line, a `length < 2` segment guard, and a third bespoke regex for `vertical-align` at `:1503`. Any mismatch silently yields `{}` — a whole clipboard's class styles vanish with no signal, which is what the WPS fixture does today. A real CSS-rule parse over the `<style>` text (selector + body per match, no index pairing) would end the per-variant patching. **M**
...
- [ ] **`copy-action-image` has no producer** — `paste.ts:1356` branches on it, and nothing in the repo writes that marker (upstream's image copy was removed). Dead arm; delete it rather than keep migrating it. **S**
- [ ] **The Insert-function dialog has empty and unreachable categories** — the always-empty `Luckysheet` tab was deleted on 2026-08-28, but `t: 4/5/7` are still empty (no `functionlist` entry uses them) while `t: 15` (14 functions) and `t: 17` (12) have no tab at all, so those are reachable only by search. `filteredFunctionList[selectedFuncIndex]` would also throw on Confirm in an empty category. **S**
- [ ] **Dead upstream comment-system CSS** — `.sheet-postil-*` (23 selectors, `SheetOverlay/index.css`) styles the fortune-sheet "postil" comment dialog that SHEETS.md records as fully removed in favour of Eigen comment cards. Confirmed unmatched in a live editor session on 2026-08-28. **S**
...
| HTML paste nulls row heights | **Fixed** in `554f6f9f5`. A height is written only when the source `<tr>` supplies one. Note the stored value was `0`, not `null` — `setRowHeight` runs `Number(null)` — which is exactly what `to-xlsx.ts:151` multiplied into a zero-height row |
| HTML paste drops inline underline | **Fixed** in `554f6f9f5`. `cell.un` ORs in `td.style.textDecoration` using the `bl`/`it` idiom |
...
| Filter menu can crash on a tall sheet | **Fixed** in `4839cfff7`. The three `without(x, ...arr)` spreads are Set differences; `without` is gone from the imports |
