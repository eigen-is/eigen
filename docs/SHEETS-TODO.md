...
Verified, user-visible, bounded. Both remaining entries carry a small design choice, so neither is decision-free like the seven fixed on 2026-08-28.
...
- [ ] **The link range-selection modal is found document-wide** — `onRangeSelectionModalMoveStart` and `onRangeSelectionModalMove` (`state/modules/hyperlink.ts:173,191`) query `document` for `div.sheet-link-modify-modal.range-selection-modal`, so with two workbooks mounted, dragging B's modal moves A's. This is the unfixed half of the Ctrl+Shift+F bug closed on 2026-08-28. MoveStart can reach the right element from its own `MouseEvent` target; **Move cannot** — it fires from grid-overlay mousemove (`state/events/mouse-drag.ts:383`) and its only other argument is `globalCache`. Fixing one half alone makes the pair incoherent (B's geometry applied to A), so either thread the element through `globalCache` or leave both. **S** once that shape is chosen
...
| HTML paste loses Excel-for-Mac class styles | **Fixed** in `554f6f9f5`. The class-block parser is whitespace-agnostic (`/^\S.*/gm` + `.split(/\n\s*/)`); both committed fixtures are now positive tests |
