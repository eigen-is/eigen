...
- The `sheet-copy-action-table` marker string is load-bearing in three modules and a committed test fixture. It was renamed from `fortune-copy-action-table` on 2026-08-28 with the gap accepted (HTML copied before that deploy falls through to the foreign-HTML parser); don't move it again without the same deliberate call.
...
| Filter buttons disagree with the column they open | **Fixed** in `4839cfff7`. `createFilterOptions` skips `config.colhidden` columns, so draw and hit-test come from one list |
...
| Ctrl+Shift+F focuses the wrong workbook | **Fixed** in `5df33367a`. The handler focuses the `cellInput` it was handed. The toolbar half was deleted rather than scoped: nothing renders `.fortune-toolbar`, and the shortcut is a focus *lock* ("Toggle sheet focus lock" in the locale), not a two-way focus move |
