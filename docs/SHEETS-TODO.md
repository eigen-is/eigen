...
- [ ] **Drag-to-move a selection is unreachable** — `.sheet-cell-selected` carries `pointer-events: none` (`SheetOverlay/index.css`), so the `onMouseDown` → `onCellsMoveStart` wired on that same div (`OverlayVisuals.tsx:151-163`) can never fire. Its only `pointer-events: auto` child is `.sheet-cs-fillhandle`, which is why dragging the corner still extends the range while dragging the border does nothing — the symptom a user reports as "I can only resize the selection, never move it". The whole `move-cells.ts` module (drag-replace, its `.sheet-cell-selected-move` preview outline, the Escape/undo paths in `keyboard.ts:337`) is dead code today. Pre-existing, unrelated to the 2026-08-28 rename (verified on `main`). Fix shape needs a decision: four thin `pointer-events: auto` edge strips that start the move, versus dropping the feature and deleting `move-cells.ts` — a click *inside* a selection must keep starting a new selection either way. **M**
