...
Single source of truth for `packages/sheet/` pending work. Goal: make the
...
1. **Enable biome on `state/`** — ✅ done on `biome-state-cleanup` (merged
   2026-05-10). 218 files in the package at 0 errors / 0 warnings; typecheck
   green across all 12 workspaces; 749 sheet-package tests + 1189 API tests
   ...
   **Tightened on `biome-state-cleanup` (2026-05-11)** — landed alongside Group F:
   ...
   - `EditorSheetConfigExtras` hoisted to `engine/types.ts` to dedupe the
     ...
     `sheet → lib` direction rule). Standardised key-type on
...
6. **`from-xlsx.ts::parseA1` duplicates engine's `parseA1`** — ✅ done on
   ...
   `@workspace/sheet/engine`; `buildMergeStructures` now destructures
...
10. Move package to `apps/sheets/src/sheet/` — only `apps/sheets/`
...
`apps/sheets/src/sheet/` to make the dependency explicit. Low
