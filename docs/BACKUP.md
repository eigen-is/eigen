...
- **Every container's `data.db` and `comments.db`** (eigendocs, sheets, slides, stickies, vector drawings, chats), taken freshest-first: an open document's live handle first, then a pending staged upload, then the stored object. A backup taken during an S3 outage holds the newest local bytes, not a stale remote object. A crash temp an unclean shutdown left in `tmp/` is not read, so edits that live only there are not in the archive.
