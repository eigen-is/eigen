...
- **Mail as Maildir files**, **contacts as the `.vcf` cards themselves** and **calendar as the `.ics` files under `eigen.calendar/calendars/`**, so all three survive a rebuild of their index database. Empty directories come along too: a mailbox nobody has ever been delivered to still needs its `new/` and `cur/`, because that is what the mail sync watches, and an empty calendar directory is the only thing that brings its calendar back.
...
The same gap has one shape worth naming, because it is a duplicate rather than a miss: the file walk and each `VACUUM INTO` happen at different instants and neither takes a domain's write lock, so an event moved between two calendars in between — one rename — can be archived under both the calendar it left and the calendar it joined. The two copies share a UID but not a calendar, which is the scope the calendar index dedupes in, so the restored home keeps both and the copy rule mints fresh event ids for the second one ([CALENDAR.md](CALENDAR.md) § Reconcile vs. rebuild). The user sees the event twice and deletes one.

**A restore does not put mtimes back.** The tar carries them, but the extractor writes every file with the clock of the restore, so on the first open of a restored home every card and every `.ics` looks drifted and is re-read and re-hashed. Bytes that still hash to their stored etag settle as the restores they are — stats refreshed, no ctag bump, no client re-download — so the cost is one pass over the files, not a resync for every device.
