...
The problem this proposal set out to solve: the [setup guide](../../docker/SETUP-GUIDE.md) said everything runs in Docker, yet install and update both needed host-side Bun: `bun install`, a full frontend build, and `scripts/update.sh` re-running all of it. That is fine for a developer on a VPS. On a NAS it is the wrong shape: Container Manager is the supported way to run software there, and every fresh install compiled the whole monorepo on the target machine, which 2 to 4 GB machines ran out of memory doing.
...
4. **Data and backup directories.** `mkdir -p data backups`. For each one that is empty, one root container sets its ownership: `docker run --rm -v "$PWD/data:/data" alpine chown 1000:1000 /data`, one directory, not recursive. Then a write test as `--user 1000:1000`. An existing, non-empty folder is never touched; the old `chown -R` in `setup.ts` moved out into this step, since it cannot succeed inside a non-root container anyway. Never `chmod 777`, never recurse through a Synology shared folder.
