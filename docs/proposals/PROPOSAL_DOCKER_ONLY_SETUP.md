...
| `scripts/generate-env.sh` | A second, Bash-only generator of `.env.production`, listing every `VITE_APP_*_URL` again. | Two copies of the deployment rules that must be edited together. Do not add a third; make this a thin wrapper. |
...
| `docker/static/Dockerfile` and the `caddy` service in [`docker-compose.yml`](../../docker-compose.yml) | Static image copies a host-built `dist/`; edge Caddy bind-mounts host `./dist`, `./Caddyfile`, and `docker/caddy/*`. | Both web modes depend on a host build. Changing only the static Dockerfile leaves the default edge mode broken. |
