...
**Moving an existing install.** eigen.is and demo.eigen.is move over by hand, since no shim replaces the deleted scripts: a `git pull`, then `./eigen setup` and from then on `./eigen update` ([SELF-HOSTERS-FIRST.md](../SELF-HOSTERS-FIRST.md) block 1 lists what changes). The host's `dist/` and `node_modules` become leftovers `./eigen update` names and leaves alone.
