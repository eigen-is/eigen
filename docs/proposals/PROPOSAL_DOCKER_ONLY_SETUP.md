...
2. **Network snapshot.** The launcher writes `docker network inspect $(docker network ls -q)` to a temporary file and mounts it read-only. The configurator reads that file (`EIGEN_DOCKER_NETWORKS`) and picks a subnet only then; without it, Compose's default stands. Subnet selection stays in TypeScript, including the rerun rule that never shifts a live deployment's subnet. A failed inspect aborts; it is not "no networks".
...
`eigen update` takes a snapshot before anything changes. The current image's CLI resolves the target version to image digests, and the `sh` launcher pulls every image first. Then the NEW image's CLI performs the update: it backfills new keys into `.env.production` by rerunning setup non-interactively, takes a snapshot, keeps the previous release's digests and Compose files beside it, rewrites the launcher and Compose files from the new image, and runs `docker compose up -d` with the same project name and data locations. New upgrade logic therefore runs on the update that ships it. No `git pull`, no `bun install`, no host build. A failed pull leaves the running stack untouched. The API's 30 s `stop_grace_period` and upload drain stay as they are.
