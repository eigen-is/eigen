...
The operator learns one command, `eigen`, with subcommands for setup, update, backup, restore, password reset, status and logs. The host part is the thin POSIX script described here. Everything interactive runs inside the API image as a Bun CLI, so prompts can use a proper terminal UI without installing anything on the host. Below, `setup.sh` is `eigen setup` and `update.sh` is `eigen update`; [SELF-HOSTERS-FIRST.md](../SELF-HOSTERS-FIRST.md) block 1 has the full command list.
...
`setup.ts` stops printing build commands; when `EIGEN_DOCKER_NETWORKS` is set the launcher owns the next steps, and `bun run setup` on a developer machine points at `setup.sh`.
...
**One Bun pin.** Both Dockerfiles take `ARG BUN_VERSION` with no default. The build overlay passes `${BUN_VERSION}`, which `setup.sh`, `update.sh`, the test harness, and CI read from `.bun-version`. The file stays the single source.

**Build context.** `.dockerignore` keeps excluding `data/`, `caddy-data/`, backups, and every private env file, starts excluding `dist/` and `**/node_modules`, and starts including `CHANGELOG.md`. Only public build settings are passed as build args; never a production env or an SMTP password.
...
The Compose model inverts: `docker-compose.yml` references images, and a `docker-compose.build.yml` overlay adds the `build:` blocks for source installs. `scripts/release.ts` attaches a bundle: the Compose files, the `eigen` launcher, `docker/fail2ban/`, `.env.example`, a small manifest (version, image digests, whether the release breaks a persisted format), and checksums. A release install needs no Git, no Buildx, no registry login, and no source checkout. Images are published before the bundle is, and the bundle pins the digests it was tested with; nothing deploys a moving `latest`.

**The release gate.** Before the bundle is attached, a CI job runs on a clean runner with Docker and no Bun: install the previous release's bundle non-interactively, create a document, run the new bundle's `update.sh`, and assert the stack is healthy and the document is still there. Then a fresh install of the new bundle alone. An update that breaks is worse for a self-hoster than a missing feature, and the env-backfill breakages show the class recurs; this is the test that would have caught them.
...
`update.sh` becomes plain `sh` and gains a release mode: download the matching bundle, verify checksums, re-exec the new `update.sh` (the rule the script follows today after `git pull`, so new upgrade logic runs on the update that ships it), `docker compose pull` everything first, rerun `setup.ts` non-interactively to backfill new keys into `.env.production`, run `scripts/backup.sh`, then `docker compose up -d` with the same project name and data locations. No `git pull`, no `bun install`, no host build. Source mode keeps working through the build overlay. The API's 30 s `stop_grace_period` and upload drain stay as they are.

What happens to today's other steps: the Caddy reload goes away, because the Caddyfile lives in the image and a new image recreates the container. The fail2ban filter refresh and jail reload stay, fed from the bundle's `docker/fail2ban/`. Pruning narrows to Eigen's own superseded images, selected by their OCI source label; the build-cache prune runs in source mode only.

**Breaking releases.** Before 1.0 a release may drop or convert a persisted format ([ROADMAP.md](../ROADMAP.md) states the policy). The bundle manifest flags such a release; `update.sh` prints the release note and asks for confirmation before it touches anything, and refuses in a non-interactive run without an explicit flag. A self-hoster must never learn this from missing data.

**Moving an existing install.** A source install such as eigen.is runs the old `update.sh` once more; it pulls, re-execs the new script, and that run must succeed with no Bun on the path. The host's `dist/` and `node_modules` become leftovers the script names and leaves alone.

[`backup.sh`](../../scripts/backup.sh) archives `data/` and `.env.production` after stopping the API. `caddy-data`, the `postfix-queue` volume, and `data/certs` are outside it, and Postfix and Dovecot keep writing while it runs. Whole-stack recovery is the [backup proposal's](PROPOSAL_BACKUP_RESTORE.md) phase ③; this proposal only requires that the update path calls the snapshot, switches to the phase ③ backup once it exists, and that a rollback keeps the prior release's bundle and snapshot together, because rolling images back after a schema change is not safe on its own.

## Delivery plan

| Milestone | Deliverable | Done when |
|---|---|---|
| 1. No host Bun | `setup.sh`, the network and ownership seams in `setup.ts`, the mail question with its relay prompts, the frontend Dockerfile for both web modes, runtime-only `.env.production`, the single Bun pin, `update.sh` source mode through the build overlay, the harness off host Bun | A fresh install and an update succeed on a host with Docker and Compose but no Bun, Node, or `dist/`. Every profile combination, the two mail-off ones included, passes `docker/test-deployments.sh`. eigen.is has made the move. |
| 2. No local build | The publish workflow, image-based Compose with the build overlay, domain-neutral frontend build, checksummed bundle with its manifest attached by `release.ts`, `update.sh` release mode, the release gate | Install and update from a bundle without Git, a compiler, or `docker build`. The same image digests serve two differently configured hostnames. The release gate passes in CI on the previous-to-new upgrade. |
...
| Operations | A failed pull leaves the running stack intact; a half-finished setup reruns cleanly; upgrade from the previous release keeps data and config; a release flagged breaking stops for confirmation; restore brings back a known document and, where enabled, mail. |
