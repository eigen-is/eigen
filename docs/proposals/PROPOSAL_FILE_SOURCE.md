...
### Exports, with the same choice

Every export is a direct download today: Docs, Sheets and Slides export to docx, xlsx and pdf through `useExportDocument`, Contacts exports vCards through `useExportContacts`, and both fetch the bytes with credentials and hand the blob to `downloadBlob` (`packages/lib/src/core/download.ts`). The user never gets to choose Drive, and the vCard branch mirrored the document exports rather than invent a picker of its own. With the registry in place an export is one more subject: a third builder, `subjectFromBlob(blob, name)`, wraps the response in an object URL (`embedUrl` and `downloadUrl` both `blob:`), takes the mime from the response, the name from `Content-Disposition` through `filenameFromDisposition`, and has no `drive`. The export hooks then run `save-to-drive` or `download` through the runner instead of calling `downloadBlob`, so every export menu offers Save to Drive… beside Download. Save to Drive uploads the blob through the existing multipart upload route (`useUploadFile`), the D5 shape: the bytes are already in the browser and no new seam is needed. The object URL is revoked when the picker closes or the download starts. This lands after phase 3 as its own unit, with the export menus of the three document types and the two Contacts export entries on one screenshot round.
...
| D12 | Exports get Save to Drive through `subjectFromBlob` and the same runner, after phase 3; until then every export stays a direct download. | One choice on every surface, the bytes are already in the browser, and the upload route exists. Asked for on 2026-09-13 when the vCard export shipped as a plain download like the document exports. |
