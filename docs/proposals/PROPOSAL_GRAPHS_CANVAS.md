...
**The canvas engine is ready for a new kind.** Adding one is five steps ([CANVAS.md](../CANVAS.md) § The registry): add the type to `VectorElementType` and its element type to the `VectorElement` union (`packages/lib/src/vector/types.ts`), write one `defineKind` file in `packages/lib/src/vector/kinds/`, add its line to `ELEMENT_KINDS` (`kinds/index.ts`), and add an `ELEMENT_KIND_UI` entry (`packages/ui/src/components/vector/kinds/index.tsx`). Both registries are keyed by every `VectorElementType`, so TypeScript rejects a half-added kind. `render(el, ctx)` is synchronous and DOM-free and returns an SVG string, which is why `sceneToSvg` and the compositor run unchanged in the transform Worker. The painter helpers (`renderRoughShape`, `baseRoughOptions`, `fillDefs`, `svgId`, `drawableToSvg`) live in `kinds/render-utils.ts` and are not barrel-exported. `arrow-render.ts` is the precedent for a render module beside its kind.
...
Rulings that apply: **no backwards compatibility for sheets** (Reinder, 2026-09-03), so `charts` enters the snapshot without a migration. The `.eigenvector` and `.eigenslides` shapes are not frozen and no persisted format carries backwards compatibility before 1.0 ([ROADMAP.md](../ROADMAP.md)), so the `chart` kind needs no migration either.
