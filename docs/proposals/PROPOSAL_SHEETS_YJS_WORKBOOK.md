...
Two things about the current design are worse than the docs said, found while grounding this proposal:
...
Both are recorded in [SHEETS-TODO.md](../SHEETS-TODO.md).
...
- Row and column moves become one array operation.

## What to do first

The op log can be made honest for a few small changes. Each one is independent, and each one is a property the user notices.
...
2. **Transform concurrent inserts and deletes** (M). When a peer's insert or delete marker arrives, shift the local batches that the array orders after it, and shift the incoming marker against local markers ordered before it. Standard operational-transform bookkeeping on two axes per sheet.
3. **Server-side compaction** (M). The API already has everything it needs: it holds the document, it can decode the snapshot, replay the ops and encode a new snapshot. Do that in a transform worker when the last subscriber leaves and when the array passes a threshold, and delete exactly the items that were replayed so concurrent pushes survive. Then the tab flush becomes a nicety, the log is bounded, and a killed tab loses nothing. Stop remounting the grid on a peer's snapshot while loaded; the client already holds snapshot-plus-ops.
...
After these four, sheets behaves like a shared editor in every case a user can see, and the code stays the code we have. Together they are about one week of agent time.
...
- Sheets sync and undo today: `apps/sheets/src/components/sheets/hooks/use-sheet.ts` (`handleOp`, `handleOps`, `flushSnapshot`, the `wsconnected` gate), `packages/sheet/src/components/Workbook/index.tsx` (`setContextWithProduce`, `handleUndo`, `handleRedo`, `reduceUndoList`), `packages/sheet/src/state/utils/patch.ts` (`patchToOp`, `opToPatch`, `sheetMetadataOps`), `packages/sheet/src/engine/rowcol.ts` (`shiftCellKeyedForInsert`, `shiftFormulasAcrossSheets`), `packages/sheet/src/engine/formula-shift.ts` (`functionStrChange`), `apps/api/src/lib/document/sheets.ts` (`readSheetsFromDoc`, `writeSheetsSnapshotToYjs`).
