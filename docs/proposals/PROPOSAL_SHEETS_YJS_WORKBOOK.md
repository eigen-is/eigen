...
Each item is independent, and each one is a property the user notices. Three are small. The second is not.
...
2. **Transform concurrent inserts and deletes** (L). Two peers inserting rows at the same time, or one editing a cell while the other inserts a row above it, diverge today: each client applies its own batch first, and the other's batch arrives in coordinates that no longer hold. Making this converge is operational transform, and three things make it bigger than it looks. First, a flat array cannot tell a concurrent batch from a later one, and a joiner or the API replaying the log has to know which batches to transform. So every batch must carry the id of the last batch its author had applied, a small wire-format addition. `replaySheetsOps` then becomes a transforming replayer, and the live client runs the same code on receive. Second, every positional op needs shifting, not only markers against markers: cell paths, the `"r_c"` keys in `borderInfo`, `merge` and `hyperlink`, row and column size keys, and the ranges in conditional formats, validation and filters. Third, a marker batch today ships the author's whole `config`, `hyperlink`, `dataVerification` and every other sheet field as authoritative replaces (`sheetMetadataOps`). Those are a snapshot of the author's state and cannot be transformed; a concurrent marker's replace lands last and clobbers the other's shift, so `config` no longer matches `data`. The replaces have to go, with receivers re-deriving the config shift the way they already re-derive the `data` shift, and only what is not derivable still shipped: restored cells and `calcChain` on a delete-undo. Decide this item on its own. It is the one place where Shape 2 converges by construction and the op log only by bookkeeping.
...
Items 1, 3 and 4 are about one week of agent time together and leave concurrent inserts as the one known divergence. Item 2 is another one to two weeks. After all four, sheets behaves like a shared editor in every case a user can see, with the code we have.
