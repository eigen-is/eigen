...
**The linked-copy discriminator is `data.organizer` resolved against the Home's identity.** `updateEvent` treats a row whose organizer is neither the Home's owner id nor its address (`isInvitationFromOthers`, shared with the frontend) as an attendee-side copy and discards every input except reminders and color. `parseIcs` sets `data.organizer` from any `ORGANIZER` property, including the user's own address as Apple Calendar writes it on an event with invitees, which is why the comparison is needed. `findLinkedEvent` and the RSVP relay look linked copies up by the `organizerEventId` + `organizerUserId` columns, and `moveEvent` re-homes an event under the same id.
...
- **The discriminator change is a behavior change.** Today an `ORGANIZER` that is neither the Home's owner id nor its address locks a row; after § 3 only a stamped or foreign one does. Phase 3 tests it first, because it is the path Apple Calendar takes for every event with invitees.
...
New findings, each folded into the section it belongs to: backup and restore (§ Current state, § 8), the `currentVersion` collision with the import proposal (§ 2), path validation of client-chosen names (§ 2), and the missing `.ics` file story (§ Files everywhere).
...
2. **The `ORGANIZER` lock is fixed separately, on today's storage.** It was a focused behavior bug, not a reason to wait for new storage. Organizer parsing stays — iMIP and the attendee-side protections read it — and identity is resolved in context instead: an organizer that is the Home's own owner id or address is an organizer-side event.
