...
| a cancelled occurrence | `EXDATE` on the master stays authoritative for *which* occurrences are excluded, as today, and beside it one `X-EIGEN-EXDATE;X-EIGEN-ID=<uuid>;X-EIGEN-SEQ=<n>[;TZID=…][;VALUE=DATE]:<the EXDATE's own value>` per exclusion carries the exclusion row's id and the `SEQUENCE` the RFC 5546 replay guard in `cancelInvitationOccurrence` compares. The id is kept because an attendee copy's `organizerEventId`, and the notification link built from it, can name an occurrence-only exception that is cancelled later. A missing stamp falls back to the **master's** sequence, which is what the parser already does; a stamp whose value has no `EXDATE` is dropped, because the client undid that deletion. A missing stamp also mints a fresh id. A cancelled exclusion's attendee list is not preserved: its only reader falls back to the master's. |
...
| Question | Answer |
|---|---|
| An `EXDATE` carries no sequence, so what keeps a cancelled occurrence's replay guard? | One `X-EIGEN-EXDATE` beside each `EXDATE`, carrying the exclusion's id and sequence; a missing stamp falls back to the master's sequence and a fresh id (§ 3). |
...
- **Eigen-owned per-event state rides in the file** as `X-EIGEN-*` and is re-stamped on PUT; calendar-level state stays in the index, and there is no per-calendar metadata file. Ids stay random UUIDs read back on rebuild; exclusions carry their id and sequence on an `X-EIGEN-EXDATE` stamp.
