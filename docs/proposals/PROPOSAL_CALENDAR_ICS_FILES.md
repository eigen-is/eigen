...
2. **Fix the `ORGANIZER` lock separately.** This is a focused behavior bug, not a reason to wait for new storage. Do not simply remove organizer parsing globally: iMIP and attendee-side protections depend on it, and identity must be resolved in context.
