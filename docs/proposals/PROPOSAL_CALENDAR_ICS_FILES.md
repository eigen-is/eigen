...
- **Changing the invitation model.** Linked copies in attendee calendars, server-side RSVP propagation, iMIP for external addresses, share propagation and team calendars all keep their semantics. They change writers, not behavior.
...
- `writeResourceFile(fs, path, bytes)`: temp → fsync → rename via `LocalFilesystem.writeAtomic`, temp-file cleanup at init (`cleanupTempCardFiles` generalized).
...
- `sanitizeCardUri` and `uriKeyOf` (case + NFC fold) generalized to take the required extension, so calendar passes `.ics` where contacts passes `.vcf`.

The core has two consumers, contacts and calendar, and is named and scoped as such. Mail is deliberately not one: its mutation unit is a rename that changes identity-bearing path components (`setFlags` rewrites the `:2,` suffix, `move` relocates between mailbox directories), it has no per-resource ETag, ctag or sync generation, and its reconcile exists for a foreign writer rather than for crash recovery. What all three share is one level down and already exists: `LocalFilesystem.writeAtomic`, `Semaphore(1)`, `ManagedDatabase` and the rule that the index rebuilds from the files.

Contacts is refit onto the core in the same phase with zero behavior change; the existing contacts and CardDAV suites gate it. The contact-specific pieces (labels, the avatar cache, the self card, quota) stay in `contacts/`. This phase lands on its own branch before any calendar work starts.
...
`<calendarId>` is the existing calendar id (client-chosen through MKCALENDAR, sanitized by `sanitizeCalendarId`, or a UUID). `<uri>` is the CalDAV resource name including its `.ics` suffix, client-chosen on PUT and `<uid>.ics` for Eigen-created events, sanitized the way card uris are. Overrides live inside the master's file, so the separate `<uid>-exc-<date>.ics` uris exception rows carry today go away. Team calendars use the same layout under `data/team/<teamId>/eigen.calendar/`. Deleting a calendar removes its directory after the index rows.

**Both path segments are client input and become filenames.** Today `uri` is a percent-decoded, length-capped DB column (`caldav/resource.ts`) and `calendarId` is sanitized at MKCALENDAR only (`caldav/proppatch.ts`), not on a PUT into an existing id. The store validates both at its own seam, on every entry path (CalDAV, REST, iMIP, relay, reconcile): no `..`, no `/` or `\`, no control characters, no leading dot, the required `.ics` suffix, and the case + NFC fold `uriKeyOf` applies to card uris. A name that fails is a 400 at the DAV edge, never a rewritten name, because the client addresses the resource by the name it chose.

`calendar.db` (`db-config.ts`, one new version that drops the old tables and creates the new shape; existing rows are lost by decision). `currentVersion` is `1` today and [PROPOSAL_CALENDAR_IMPORT.md](PROPOSAL_CALENDAR_IMPORT.md) claims version 2 for its additive `subscription` column: this proposal lands first and owns version 2, with `subscription` created as part of the new `calendars` shape, so the import work adds no migration of its own.
...
| exception link (`parentEventId` + `recurrenceDate`) | An override VEVENT in the same file with `RECURRENCE-ID` naming the original occurrence (the existing `computeOccurrenceTimes` rule); `parentEventId` is the master's `X-EIGEN-ID`. A canceled occurrence is an `EXDATE` on the master, no override VEVENT, exactly as today. |
...
CalDAV scheduling behavior needs an explicit decision. Today `handlePut` passes `userId` as creator metadata but does not pass the optional `user` argument to `createEvent`/`updateEvent`, so those calls do not trigger their invitation fan-out. Adding it in `putResource` would be a behavior change, not preservation of an existing trigger.

### 8 — Reconcile and rebuild

`Calendar.init` runs the § 1 reconcile per calendar directory: stat-only against `events.mtime/size`, re-indexing only changed files, dropping rows whose file is gone, and the full rebuild (rehash every file, drop tombstones, bump ctag, rotate `syncGen`) when the `calendars` row for a directory is missing or on demand. There are no fs-watchers, for the same reason contacts has none: nothing writes these files out of process. A user who edits a file by hand sees it on the next open. With one writer the `(mtime, size)` compare can be trusted outright: there is no foreign process to race. The first open after a per-home restore is the exception to "no parse": restore does not preserve mtimes, so every file re-hashes once. A matching hash updates the row's `mtime` without a ctag bump, a tombstone change or a `syncGen` rotation, so a restore costs one parse pass and no client resync.

### 9 — What stays out of the file, deliberately

Calendar name, color, default flag, visibility, the share list and the recipient-side `shared_calendars` rows are calendar-level, not event-level, and stay in `calendar.db`. iCalendar has `X-WR-CALNAME`/`X-APPLE-CALENDAR-COLOR` conventions for exports, but writing them into every event file would put one fact in N places. The calendar-level facts are small and few, and the ones that matter to another Home (name, color, permission) already travel to the recipient's `shared_calendars` row through `propagateCalendarShare`; they are not worth a second file format.
...
2. **The primitives.** `LocalFilesystem.writeAtomic`, `Semaphore(1)`, `ManagedDatabase`, one path-segment validator for client-chosen names, and shared FE/BE byte ceilings.
...
A single writer makes the calendar design cheaper in three places: no watcher coherence, no races against an external mutation, and a reconcile that can trust `(mtime, size)`. It does not touch the crash-recovery blockers, which are about the API's own kill window.

## Files everywhere: import, export and quick look

The vCard build ([PREVIEWS.md](../PREVIEWS.md), [CONTACTS.md](../CONTACTS.md)) set the pattern for a standard file that belongs to a domain: a loose `.vcf` in Drive, in a mail or in a chat gets a quick look, an import action and an export, all through one `FileSubject` and one `FILE_ACTIONS` registry. `.ics` and `.eml` should read the same way. State today:

| | Quick look in Drive | Quick look as a mail part | Quick look in chat and cards | Import into the domain | Export one item | Export the collection |
|---|---|---|---|---|---|---|
| `.vcf` | yes | yes | yes | yes (raw body + from Drive) | yes | yes |
| `.ics` | as code text only | no: `isCalendarPart` filters the part out of the chips, only the invite widget shows | as code text only | DKIM-verified iMIP only, no user action | no | no |
| `.eml` | fallback card | fallback card | fallback card | no (`/mail/deliver/:to` is localhost-only) | yes (`/message/:id/download`) | no |

What the `.ics` column reuses verbatim: a `PreviewMode` member and a `getPreviewMode` predicate, the route pair (`/drive/.../ics-preview` cached under a format tag, `/mail/.../attachment/:index/preview/ics` uncached, two segments past the index), the one-shot transform Worker job, an `assertIcsPreviewable` gate with a shared byte ceiling, a result type in `types/preview.ts`, an `import-to-calendar` row in `FILE_ACTIONS`, and the paired from-Drive and from-URL import hooks. The preview routes go through `plainApi`: an all-day `DTSTART;VALUE=DATE` is a bare `YYYY-MM-DD`, which the Eden reviver turns into a `Date`. `ATTACH` with a URI is dropped from the preview payload for the reason `PHOTO;VALUE=uri` is: an untrusted file must not make the viewer's browser fetch a URL of its choosing.

What is calendar-specific: a file holds N VEVENTs plus recurrence, so the payload is a capped list of events with their next occurrences, with `dropped` and `total` counters; a `METHOD:REQUEST`, `REPLY` or `CANCEL` file renders as an invitation and a method-less one as plain events; the import action needs a target (new or existing calendar), for which `SaveToDrivePicker` is the precedent; and no shared event card exists yet. `apps/calendar/src/components/event-detail-dialog.tsx` and the mail-local `calendar-invite-widget.tsx` would become the second and third renderings of an event, so an `EventDetailCard` in `packages/ui`, the counterpart of `ContactDetailCard`, comes first.

How it meets this proposal: export is where the storage move pays. Today the only `.ics` egress is CalDAV GET and it is lossy, so an export route built now would ship a lossy artifact and be rebuilt later. With the file as the truth, exporting one event is a byte copy of its resource and exporting a calendar is a concatenation of VEVENTs and VTIMEZONEs under one VCALENDAR; both strip the `X-EIGEN-*` stamps. Import writes resources through `Calendar.putResource` with stamps discarded and minted fresh, the same rule a client PUT gets, with one difference: a foreign `ORGANIZER` does not earn an `X-EIGEN-ORGANIZER-USER` stamp, so an imported event is a plain owned event and stays editable. That is the import proposal's "strip `data.organizer`" rule expressed on the stamp, which leaves the property itself in the file. Quick look and import do not depend on the storage move and can ship before it on today's `parseIcs`; [PROPOSAL_CALENDAR_IMPORT.md](PROPOSAL_CALENDAR_IMPORT.md) § File import is the place that work is specified, through the registry rather than a bespoke upload dialog. Un-filtering calendar parts in the mail reader also gives compose a representation for an attached `.ics`, the open cheap-wins row in [ROADMAP.md](../ROADMAP.md).
...
- **Reconcile on open is stat-only.** One `readdir` per calendar directory, one `stat` per file and an `(mtime, size)` compare; no parse unless a file changed. A cold open of a 10k-event calendar is 10k stats and one indexed query, the same shape contacts has. The first open after a restore re-hashes every file once (§ 8).
...
| Phase | Scope | Gate |
|---|---|---|
| −1 | Fix the `ORGANIZER` lock on today's storage: `updateEvent` (`calendar/calendar.ts:480-487`) treats any row with `data.organizer` as an attendee-side copy, and `parseIcs` sets it from the user's own address, so an event with invitees created in Apple Calendar is locked against its own client. Resolve identity in context (own address = organizer side); do not drop organizer parsing, iMIP and the attendee-side protections read it. | A CalDAV round-trip test: PUT an event whose `ORGANIZER` is the Home's own address, PUT a changed title, GET shows the change. Ships on main, independent of everything below. |
| 0 | Extract `core/indexed-file-store.ts` from contacts; refit contacts onto it. No calendar changes. | Contacts + CardDAV suites green, byte-identical vCard behavior, `CONTACTS.md` pointer updated. Own branch, merged before phase 1 starts. |
...
- **The discriminator change is a behavior change.** Today any `ORGANIZER` locks a row; after § 3 only a stamped or foreign one does. That fixes client-created organizer-side events and is what phase 3 tests first, because it is the path Apple Calendar takes for every event with invitees.
...
Every claim this proposal makes about current code was re-verified and still holds: `calendar.ts` is 1,628 lines with only `init`, `updateCalendar`, `deleteCalendar` and `destruct` async; `icsBlob` is declared and has no reader or writer; `EVENT_MAX_BYTES` is 20 MiB; `handlePut` has no UID-uniqueness check and does not pass `user`, so a CalDAV PUT never fans out invitations; GET and every `calendar-data` REPORT regenerate through `eventsToIcs`; the sync token is `urn:eigen:sync:<ctag>`.

| Blocker | Status |
|---|---|
| Rebuild fidelity | Open, unchanged. |
| Resource identity | Open, wider: add the path validation of `uri` and `calendarId` (§ 2). CardDAV's `no-uid-conflict` response (`carddav/resource.ts`) is the model. |
| Move/delete recovery | Open, one more caller: CalDAV `DELETE` on a calendar collection (`caldav/caldav-router.ts`, `caldav/proppatch.ts`) reaches `deleteCalendar` too. `LocalFilesystem.rename` and `delete` still do not fsync the directory. |
| Database-loss recovery | Half closed. The backup half is met by construction ([Current state](#current-state-recap)). The generation half is open as written: `contacts/reconcile.ts` computes `(book?.syncGen ?? 1) + 1`, which reuses 2 after a lost book row. |
| Editing a lossy projection | Open, slightly worse: the dialog now also stamps a timezone on a zone-less event. `getRawEventsInRange` is still index-only. |
| Invitation boundaries | Open, unchanged. |
| Shared-core contract | Open, and answered in part by [Single writer](#single-writer-what-the-three-domains-can-share): the core has two consumers, not three. |

New findings, each folded into the section it belongs to: backup and restore (§ Current state, § 8), the `currentVersion` collision with the import proposal (§ 2), path validation of client-chosen names (§ 2), and the missing `.ics` file story (§ Files everywhere). The `ORGANIZER` lock from Recommendation 2 was never fixed and is now phase −1.

## Recommendation

1. **Yes to the direction, and the route is files directly; no to implementing the current draft unchanged.** CalDAV fidelity loss is real, and independent standard files are a useful self-hosting benefit. Resolve the audit blockers before committing to the storage rewrite; architectural symmetry alone is not enough to justify its recovery complexity.
2. **Fix the `ORGANIZER` lock separately (phase −1, still open 2026-09-20).** This is a focused behavior bug, not a reason to wait for new storage. Do not simply remove organizer parsing globally: iMIP and attendee-side protections depend on it, and identity must be resolved in context.
...
5. **Raw ICS in SQLite was the smaller alternative and was not chosen (2026-09-20).** It needs the same component editing and protocol semantics and avoids only the dual-store recovery, which one writer and the contacts template bound. Files were chosen for independent file access, recovery value and one contract across mail, contacts and calendar.

## Two-step option: `icsBlob` first, files later (2026-09-11)

**Not chosen (2026-09-20).** The route is files directly. Step 1 would not have saved the hard work (component editing, the stamps, per-exclusion state and the iMIP snapshot transport are needed either way), it would have changed the storage under live CalDAV clients twice with a real-client verification round each time, and its two advantages are available without it: existing data can be kept by a one-off conversion at the version bump (see Open questions), and staying synchronous only postpones the async flip. With one writer and the contacts store as a template, the file-specific remainder is bounded. The section stays as the record of the alternative.

Step 1 makes the raw ICS bytes the truth inside `calendar.db`. One VCALENDAR per UID (master plus overrides plus VTIMEZONEs) lives in `icsBlob` on the master row, and the columns become the index over it. The `X-EIGEN-*` stamps, the ical.js round-trip serializer, in-place component editing, CalDAV GET serving the bytes verbatim with a hash ETag and PUT storing them, and the iMIP snapshot transport are all built exactly as in the file design. What step 1 does not need: phase 0 (the shared file+index core), the file store, the stat reconcile and rebuild pass, `syncGen` rotation, and the async flip of `Calendar` (SQLite writes stay synchronous). Of the audit blockers, the four about file recovery drop (move/delete recovery ordering, power-loss durability, a non-reused generation after DB loss, the shared-core recovery policy); per-exclusion state, override-only resource identity and the `data.notes`/`data.color` mapping remain.

Backward compatibility is nearly free on step 1. The column already exists and is NULL on every row, so a row without a blob is serialized from its columns on first read (or once at open) and written back. One function, no version bump that drops data, no client resync. This means the "No backward compatibility" ruling below need not be exercised for calendar.

Step 2, if ever wanted, writes each blob out as `eigen.calendar/<calendarId>/<uri>` and turns the index into the file-backed design of this proposal. The migration is a loop over rows, because the blob already is the serialized file. Everything step 1 built carries over; step 2 adds only the file layout, the reconcile/rebuild pass and the async flip.
...
- **Files directly (2026-09-20).** No `icsBlob` intermediate step. Mail, contacts and calendar follow one contract: standard files are the truth, SQLite is the index, and a loose `.ics`, `.eml` or `.vcf` can be previewed, imported and exported wherever it turns up.
...
Design decisions of this proposal (2026-09-03, revised on review the same day):

- **Files, not `icsBlob`.** The vestigial column is removed rather than revived. The two-step route was weighed on 2026-09-11 and not chosen: see [Two-step option](#two-step-option-icsblob-first-files-later-2026-09-11).
...
- **Extract the file+index core from contacts first** (phase 0) rather than clone it. Two consumers, contacts and calendar; mail is not one.
- **`Calendar` goes async.**

## Open questions

- **Keep existing events with a one-off conversion?** The 2026-09-03 ruling drops calendar data, and [ROADMAP.md](../ROADMAP.md) allows either a drop or a one-off conversion before 1.0. A conversion is cheap here: at the version bump, before the old tables go, serialize each master with its exceptions through today's `eventsToIcs`, stamp the existing ids as `X-EIGEN-ID` (so linked invitations in other Homes keep resolving) and write the files; calendars, shares and `shared_calendars` rows copy across unchanged. Nothing is lost that is not lost already, because the columns are all there is. It removes the announce-and-empty deploy on eigen.is and the cross-home orphaning of linked copies. Cost: one function and one test, deleted a release later. Proposal: do it, unless the no-BC ruling is meant to stand on principle here.
- **`data.notes` and `data.color`.** They remain public `EventData` fields, and linked-copy edits explicitly preserve color; no calendar-app writer does not prove no API consumer. Dropping them would conflict with the unchanged-REST/types non-goal. Decide their file mapping before implementation, or approve removal separately. RFC 5545 `COMMENT` is a candidate for notes; RFC 7986 `COLOR` uses CSS3 color names rather than Eigen's hex values.
