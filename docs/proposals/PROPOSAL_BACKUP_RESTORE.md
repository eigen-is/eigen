...
Phase ② followed a signed-off implementation spec that supersedes this text where the two differ. The design held; these are the drifts worth knowing when reading the rest of the page.

- **The verify pass was built here, not shared.** `verifySnapshotDb` from PROPOSAL_DATA_INTEGRITY never existed, so phase ② built `verifyFolder` (`apps/api/src/lib/backup/verify.ts`), which judges any folder with a manifest. Stage 3 is "every Yjs blob decodes, and a document that has blobs decodes into a document with shared types", not "the declared roots exist and are populated": which roots a document has depends on its type and on the app version that wrote it, and a document nobody has typed in holds no blobs at all and must not fail.
- **The job runner is net-new.** The transform runner's bookkeeping was not reusable, so `apps/api/src/lib/backup/jobs.ts` is its own in-memory job map: one job per home (a second request gets 409), an SSE poke rate-limited to twice a second, an hour of retention, a shutdown drain that waits a restore out in full, and a boot-time recovery of an interrupted one.
- **Guests are not backed up at all**, where § Teams, guests said per-guest backup was possible but pointless. Guest homes are disposable (guest cleanup deletes them), so every backup route refuses a guest ownerId with a 400, and the admin Users page never lists a guest in the first place. Phase ③ skips `data/guest`.
- **File version history is always included** (`versions/` inside a container). It is the only copy of an old file state, so it is not derived data and there is no option to leave it out.
- **The artifact is `.tar.zst`, and Eigen writes the tar itself.** `Bun.Archive.write` writes a lazy `Bun.file` entry as an empty one and buffers the whole archive (1.2 GB folder, 3.9 GB RSS), so `archive.ts` streams a pax tar into a zstd compressor and reads one back itself (`Bun.Archive` on Bun 1.3.14 also stops at, or crashes on, a non-ASCII entry name).
