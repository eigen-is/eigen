...
- **Guests are not backed up at all**, where § Teams, guests said per-guest backup was possible but pointless. Guest homes are disposable (guest cleanup deletes them), so the routes refuse a guest ownerId and the pane hides the section. Phase ③ skips `data/guest`.
