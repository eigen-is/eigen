...
- **The artifact is `.tar.zst`, and Eigen writes the tar itself.** `Bun.Archive.write` writes a lazy `Bun.file` entry as an empty one and buffers the whole archive (1.2 GB folder, 3.9 GB RSS), so `archive.ts` streams a pax tar into a zstd compressor and only reading goes through `Bun.Archive`.
