...
Contact-card writes, `.eml` imports and calendar resource writes share one gate on the data half of the budget, `enforceHomeDataQuota(ownerId, addBytes, creditBytes)`: `enforceCardBudget` (`lib/contacts/contacts.ts`), `Mail.messageImport` (`lib/mail/mail-domain.ts`) and `writeResource` (`lib/calendar/calendar-store.ts`) all run it before writing, 507 on a projection over budget. `creditBytes` is the size of the stored resource the write replaces, 0 for a create. One rule covers every edit at a full budget: a rewrite (`creditBytes > 0`) that grows what it replaces by at most `HOME_DATA_EDIT_GRACE_BYTES` (1 KiB) is never refused, however far `used` already sits above `max` — a title fix, an RSVP, an exclusion, a cancelled override, a truncated RRULE, a shrinking rewrite, on REST and CalDAV alike, because editing and deleting are how a user gets back under the budget. The overshoot is at most that grace per write. A create and a rewrite that grows by more are metered on the projection as before.
