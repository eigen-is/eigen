...
- **SMTP `tls: { rejectUnauthorized: false }`** (`mailer.ts:44`): accepts any cert on the SMTP hop. Likely intentional for an internal relay — make it env-opt-out or add a WHY comment. — **✅ RESOLVED — WHY comment**: docker-compose confirms the hop is API→bundled-postfix inside the docker network (`SMTP_HOST` defaults to `postfix`; host.docker.internal variant is host-local); postfix owns TLS toward the internet, so verification off is by design on this hop.
...
- **UUID source drift**: `chat.ts`/`mount.ts`/`maildir-store.ts` use `node:crypto randomUUID`; `calendar.ts`/`contacts.ts` use the `uuid` npm package's `v4`. `crypto.randomUUID` is a Bun global — unify on it and drop the `uuid` dep. — **✅ FIXED — post-Unit-5 cleanup**: both files on `node:crypto randomUUID`; `uuid` + `@types/uuid` dropped from apps/api, lockfile synced (`packages/sheet` keeps its own `uuid@^9` — separate cleanup domain).
