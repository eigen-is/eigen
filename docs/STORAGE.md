...
The chat wizard has one honest miss. Its `dedupeName` creates let the server suffix a colliding name (`Name (2)`), so the row that lands is not the name the client sent and no poll could find it. Those creates skip `createWithReconcile` entirely (snapshot included) and surface the failure exactly as the server returned it, for a chat that may well be in the list.
