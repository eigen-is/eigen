...
| **S3 upload-queue orphaned-PUT fix** ([deep-dive](AUDIT_DEEPDIVE_UPLOAD_QUEUE.md)) | Not started — chaos-verified 2026-07-12 (no prod code changed). Audit #2 recorded the *mechanism* as accepted risk; the deep-dive proved it is silent **and** permanent. | S–M | No — purely in-process orphan tracking, no schema/format change | A PUT that stalls past the queue's 120s timeout stays alive in Bun's `S3Client` and lands late: (1) after a newer write acks and the doc closes there is no local copy of the newer bytes left, so the object **silently, permanently regresses** (the nbg1 slow→503 class); (2) after a delete, it **resurrects the deleted bytes as an undeletable zombie** (retention/GDPR smell). Fix: on timeout register the floating write in an in-process `orphans` map, park the acked row until the orphan settles, then re-PUT the last-acked bytes (or re-issue the delete after `cancel`) — an orphan can't outlive the process, so this covers the whole hazard (~30–50 lines). Ship-first: a 5-line loud log when a late orphan settles. Red net: `upload-queue-chaos.test.ts`. |
| **CalDAV occurrence-keying follow-ups** ([deep-dive](AUDIT_DEEPDIVE_CALDAV_TZ.md)) | Unit 4 shipped the P1s (#8/#9, the new iMIP series-destruction #A/#B, #24); the correctness remainder is parked. Verified 2026-07-12. | S–M | No — no frozen-format or schema change | Remaining: **#C** RECURRENCE-ID serialized from the moved startTime → orphan override; **#D** CalDAV re-PUT never deletes stale exceptions, so "undo delete occurrence" doesn't stick; **#E** FE RSVP `scope:'this'` duplicates exception rows via UTC-date occurrenceDate; **#F** no VTIMEZONE emitted → Eigen↔Eigen iMIP shifts the instant off event-tz; **#G** floating/unresolvable-TZID datetimes parsed through the server's local timezone (prod safe by accident). Plus an **open product call awaiting Reinder**: occurrence-level iMIP changes broadcast SSE but persist no tray notification. Red tests preserved with the deep-dive. |
