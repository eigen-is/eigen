...
| Drive instant-delete mis-keys cross-owner rows | S | Pre-existing (surfaced by 2026-08-03 since-v0.1.0 review): `DriveLayout`'s fine-pointer instant delete calls `useDeletePaths(ownerId, mountId)` keyed on the LAYOUT's owner/mount, so deleting a cross-owner row in filter views (`eigendoc-list-view.tsx` passes `allowDelete` and can list teamdrive items) hits the wrong home and 404s. The coarse-pointer path via `DriveDeleteItem` keys per item and is the correct model — mirror it for the instant path. |
