...
| Repeat docx import into the same doc 409s on media names | S | `importIntoDocument` (doc arm) commits the Yjs update, then `saveDocImages` → `mount.createFile` with deterministic names (`image-0.png`, …) throws 409 against the previous import's media — content already replaced, images half-stale. The sheets arm re-imports cleanly. Fix: overwrite existing media files by name (the new update references the same names). Surfaced by the 2026-08-04 transform-workers external-review gate; pre-existing Phase 3 behavior. |
