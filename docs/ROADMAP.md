...
| **Storage-base audit fixes** ([AUDIT_STORAGE.md](AUDIT_STORAGE.md)) | Audit done 2026-07-05, probe-verified; fixes 0%. Headline findings: a user folder named `.trash` wipes all trashed bytes on path-based `local` mounts (P1, confirmed); Unicode-case clobber on case-insensitive filesystems; served file bodies cache 24h with no ETag while `paths.hash` sits unused. | S–M | No | 11 items in the doc's summary table, most an afternoon each; item 4 (chat doc-DB open/close serialization) is the only one needing design care. |
...
storage/mount/drive — base judged solid, 11 fixes queued, see the P1 row above). The 2026-07-01
