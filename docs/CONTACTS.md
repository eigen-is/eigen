...
- **`packages/lib/src/core/contacts/`** — FE hooks + SSE handlers, including `hooks/use-transfer.ts` (`useExportContacts`, `useImportContacts`, `useImportContactsFromDrive`) and `preview-lines.ts` (the two counted lines the Drive vCard preview ends on — [PREVIEWS.md](PREVIEWS.md)); shared types in `packages/lib/src/types/contact.ts`.
