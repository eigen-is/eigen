...
- **`packages/lib/src/core/contacts/`** — FE hooks + SSE handlers, including `hooks/use-transfer.ts` (`useExportContacts`, `useImportContacts`, `useImportContactsFromDrive`, and the `useVCardFile` reader the Drive detail hero parses a `.vcf` with — [PREVIEWS.md](PREVIEWS.md)); shared types in `packages/lib/src/types/contact.ts`.
