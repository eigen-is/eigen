...
**One import, one list refetch.** Every stored card broadcasts `contacts:contact-created`, so a whole-book import is a burst of them; `handleContactsSSEvent` (`packages/lib/src/core/contacts/sse-handlers.ts`) debounces each event's invalidation by 250 ms, so every open tab refetches the list once at the end instead of once per card. The importing tab's own `onSuccess` invalidation is unchanged. A batched server-side event is the follow-up ([ROADMAP.md](ROADMAP.md)).
