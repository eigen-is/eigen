...
- exceljs pairs a redundant `TargetMode="External"` rel with every location-form internal
  hyperlink it writes (its writer rels ALL hyperlinks; Excel itself authors location-only). Our
  importer is immune (location wins), but real-Excel/Google handling of the hybrid is
  unconfirmed — covered by the cycle-8 manual spot-open; if Excel mis-resolves, post-process the
  zip to strip the junk rels from location-form entries.
