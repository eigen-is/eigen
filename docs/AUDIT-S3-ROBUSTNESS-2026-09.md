...
| DL-2 | Home teardown awaited a document open parked in its GET, so one stalled GET hung every request for that owner | medium | fixed | `storage/slow-download.test.ts`: `the owner's next request gets a Home while the idle one waits on a stalled document GET`; `storage/docdb-open-close-race.test.ts`: `mount teardown settles while an open is parked on storage` |
...
| DL-8 | Whole-body reads (`arrayBuffer()`, `text()`) in previews, thumbnails, import, inline edit, transforms and extraction had only Bun's bound (missed by the audit) | medium | fixed | none |
...
| ST-3 | `StorageBackend.delete` answered `false` for absent and failed alike, and S3 sent a HEAD before each DELETE | low | contract fixed (`true` for a missing key, `false` only on failure, one request); a failed permanent delete is still only logged | `storage/storage.test.ts`: `delete returns true for a missing file`; `storage/s3-minio.test.ts`: `delete returns true, then exists false, and a second delete still returns true`; `backup/safety-copies.test.ts`: `a delete that cannot reach the bucket keeps the folder and says so` |
...
| ST-7 | `storage/versioning.test.ts` casts `drive.documents` to a `Map`; a `CollabRegistry` has no `size`, so one assertion compares `undefined` with `undefined` | low | open | n/a |
...
- **One knob**: `STORAGE_TIMEOUT_MS` (30 s) in `lib/storage/s3-storage.ts`, shrunk in tests with `setStorageTimeoutMs`.
- **HEAD, stat and DELETE** (`exists`, `size`, `delete`) race the deadline and answer `ApiError(503)`. Bun cannot abort them, so the request runs on in the background; only the caller stops waiting.
- **Streamed reads** (downloads, copy, backup capture, version snapshots through `writeTempWithHash`; whole-body reads through `readStorageFile`) run on an idle deadline reset per chunk and end with `reader.cancel()`. Gotcha: a `read()` pending when `cancel()` runs resolves `{ done: true }` rather than throwing, so the loop checks a stopped flag, or a canceled download would count as complete.
...
- **Teardown** aborts `mount.downloads` first (`closeAllDatabases`, `Drive.destruct`), so no close or Home shutdown waits on a download.
...
- **Low, no test**: UP-6, UP-7, BK-6, a failed permanent delete that is only logged (ST-3), and the vacuous ST-7 assertion.
