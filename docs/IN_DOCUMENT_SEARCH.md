...
| Slides, Vector | `packages/ui/src/components/vector/hooks/use-canvas-doc-search.ts` | `searchScene` over every element kind's `searchText` (rich text plain-texted, arrow labels), frame then z-order; match ids ARE element ids, and `reveal` hands the host the element so it can select it; the canvas itself pans the active match to the container center at the current zoom (`centerOn`, a no-op on a fitted page). Slides passes a `contextOf` that labels each match with its slide ("Slide 3"), reveals by activating that element's frame first, and rings the rail thumbnails that hold a match | — |
