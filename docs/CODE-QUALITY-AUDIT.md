...
Still open: §4 (calendar home-relay refactor + drop cargo-cult `:ownerId` from home-independent routes — `waitlist.ts` etc.), §5.2–§5.5 / §5.8–§5.10 type cleanups (including `AdminUser` / `TeamMember` local projection types that remain on files touched by the §1 PR), §6.1–§6.6 over-engineering judgment calls. The doc is otherwise still authoritative.
