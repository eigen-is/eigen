...
**2026-05-18 — §2 frontend layout & shared component usage landed** on `chore/audit-section-2`. (a) `apps/space` 7 settings routes wrapped in `ColumnLayout` + `Column` (§2.1). (b) Calendar date formatting collapsed onto `@workspace/lib/date`: new `formatMonth(date, 'long' | 'short')` for `calendar-toolbar` + `recurrence-picker`; `event-detail-dialog`'s local `formatFullDate` kept (unique `weekday + UTC` shape, no second caller). Broken-window fix across the board: every `toLocaleDateString` / `toLocaleString` now passes explicit `'en'` (in `date.ts` and `event-detail-dialog.tsx`) — Dutch users were silently getting Dutch month/weekday names in an otherwise English-only UI (§2.2). (c) 22 template-literal `className` sites converted to `cn()` across stickies / slides / docs / admin / drive / index / contacts / notification-bell / chat-slash-suggest / column-layout / mount-form / note-card-dialog / usage / eigendoc-new-button (§2.3). (d) `blog.index` `hover:text-blue-600` → `hover:text-link`; `mount-form` `text-green-600 dark:text-green-400` → `text-success` token (and dropped the now-stale "Hardcoded green is intentional" comment) (§2.4). (e) `login-fa2` raw text-link `<button>`s → `<Button variant="link" size="sm">`; chat sidebar drops "No chats yet" entirely and hides the TEAM CHATS section when no team has any chats (new `useTeamsHaveChats(teamIds)` hook in `packages/lib/src/core/chat/hooks/use-chat.ts` — keeps the multi-team `useQueries` aggregation in the hook layer, sharing cache with `useChats` via the same `driveKeys.mime` query key) (§2.7). Skipped per review: §2.5 (`conflict-dialog` has 3 actions, doesn't fit `ConfirmDialog`; `email-draft` alert is single-occurrence — extracting a wrapper would be over-engineering), §2.6 `TooltipPopoverButton` (single-file use), §2.7 `team-detail` inline `<p>` (inline section hint, not over-engineering).

**2026-05-18 — §3 hook-layer cleanup landed** on `chore/audit-section-3` (21be2535). (a) §3.9 — biggest user-visible bug fix: 9 sites of `throw new Error(String(error))` in `use-teams.ts` (5) + `use-members.ts` (4) replaced with `throw new Error(error.message ?? 'Failed to ...')`, matching the established `use-security.ts` pattern. Admin team/member flows were toasting `[object Object]` because `String(error)` on better-auth error objects stringifies the wrapper. **AppError is for Eden Treaty response shapes (`{error: {status, value}, status}`), not better-auth errors (`{message, status, code}`) — these need plain `Error` with `.message` extraction.** (b) §3.1 + §3.3 — added `adminKeys.users()` / `usersFiltered(filter)` slots (note: not org-scoped — `settingsApi.users` is server-wide); `invalidateAdminUsers(queryClient)` export; `useDeleteUser` invalidates both lists; `_auth.orphans.tsx` / `_auth.guests.tsx` drop the `useQueryClient` workaround. (c) §3.2 — `chatKeys` / `commentKeys` / `editorKeys` / `teamKeys` all gain an `owner(id)` level following the `all → owner → resource → instance` convention; `teamMountKeys` merged into `teamKeys.mounts`. Cache key shapes change (one-time miss on next mount, no runtime impact). (d) §3.4 — `fa2.tsx` `try/catch + console.error` collapsed to `try/finally` (parent's `onMutationError` was already toasting; catch was noise, not silencing); `login-fa2.tsx` migrated to new `useVerifyTotp` + `useVerifyBackupCode` hooks (`useVerifyTotp` signature widened to `{code, trustDevice?}`, success toast dropped — caller signals via redirect/step); `contact-edit.tsx` redundant `try/catch + setError` deleted (parent's `onMutationError` toasts the real `AppError` message). (e) §3.5 — notification hooks get `staleTime: 60_000` (topbar mounts often; SSE handles real changes). (f) §3.6 — `useComments` tightened: `enabled: !!ownerId && !!mountId && !!containerId`, `staleTime: 120_000`. (g) §3.7 — `useChatRoom` invite case dropped `try/catch + addLocalMessage('Failed to invite ...')`; `useInviteToChat`'s `onMutationError` covers the failure path; the throw correctly escapes the switch + outer if-block so the bottom `postMessage.mutateAsync` does not also fire. (h) §3.8 — `calendarKeys.access(ownerId, calendarId)` + `driveKeys.breadcrumb(ownerId, mountId, pathId)` slots added; inline builds replaced. Broken-window fixes surfaced by review: `useAdminUsers` was missing `enabled` (now `!isGuest`); `useChats` + `useMessages` were missing `staleTime` (both `60_000`). Skipped per review: §3.4 `setup-wizard.tsx` + `index.tsx` waitlist form — both have legitimate inline error UIs with single callsites; hookifying = over-engineering. Doc-only carve-out: §4.4 / §4.5 annotated — `settings.ts`, `setup.ts`, `public.ts`, `waitlist.ts` are home-independent and should NOT carry `:ownerId`. To be acted on in §4 session.

**2026-05-18 — §4 backend pattern consistency landed** on `chore/audit-section-4`. (a) §4.1 — calendar sharding-seam violation closed: `resolveCalendarForEvents` (which returned a foreign user's live `Calendar` instance for cross-owner callers) replaced with `checkCalendarAccess` (returns `{permission}` only). Five new seam functions in `home-relay.ts`: `pullEventsInRange`, `pullCalendarById`, `createEventAt`, `updateEventAt`, `deleteEventAt`. The five affected routes (POST/PUT/DELETE events, GET event-range scoped, GET access) now route every cross-user read/write through the relay. Multi-calendar event-range stays on `resolveCalendar` (aggregates over the caller's own/team home, no cross-user fetch). Side effects (SSE / notifications / mail / invitation propagation) all continue to fire from the owner's home — correct semantics preserved. Team-home permission check moved from direct `getHome(ownerId).calendar.checkPermission(...)` to `pullCalendarPermission(...)` for relay-discipline uniformity. Input typing uses `Parameters<Calendar['createEvent']>[1]` to avoid duplicating Calendar's inline anonymous arg type. (b) §4.3 — `SharedDrive.getMimeTypeContents` / `getMountMimeTypeContents` silent `[]` for non-team-member callers → `throw ApiError(403, 'No read permission')`, matching every other unauthorized SharedDrive method. Added `requireTeamMembership()` private helper next to the existing `withReadPermission` cluster. Updated the one mount-scoped test that asserted the old empty-array behavior. (c) §4.4 — AGENTS.md doctrine update: the `:ownerId` second-segment rule explicitly carves out home-independent routes (`setup.ts`, `settings.ts`, `waitlist.ts`, `public.ts`). (d) §4.5 — `waitlist.ts` paths went from `/waitlist/:ownerId/entries[...]` to `/waitlist/entries[...]`; all 5 hooks in `use-waitlist.ts` dropped the `ownerId` arg; query keys collapsed to `['waitlist', 'entries', status]`; `enabled: !isGuest` + `staleTime: 1000 * 60 * 2` added to match sibling admin hooks (`use-admin-users` / `use-members`); 14 test URLs updated. (e) §4.8 — added the drift-rule doc on the `Drive` class header (route surface via `getSharedDrive` vs lib-internal callers); `// Called by:` annotations on `size`, `findContainerPath`, `touchUpdatedAt`, `touchFile`, `closeCollabDocument`, `receiveACLChange`, `getSharedPathsByMe`, `getSharedPathsWithMe`, and `addMount` (the last surfaced by review — called via `TeamHome.addMount` from `/team/:ownerId/mount`); AGENTS.md "Escape hatch" paragraph added. (f) §4.9 — `setup.ts` route's `set.status = X; return {ok:false}` → `throw new ApiError(...)`; the canonical `SetupResult` discriminated-union anti-pattern (BAD example in CODE-STANDARDS.md) dropped — `completeSetup` now throws `ApiError(400)` on validation, returns `{user}` on success, with a narrowed try/catch around the better-auth external calls only (avoids the reviewer-flagged issue of mapping infra writes to 400). FE setup-wizard adapts via `AppError(res)`; two `as Type` casts on Eden Treaty responses (broken windows) dropped; test adapts via `setupResponse.ok`. Skipped per review: the calendar-existence check for team owners (pre-existing — refactor neither introduces nor amplifies); `updateEventAt`/`deleteEventAt` accepting any eventId regardless of calendarId scope (also pre-existing).

Still open: §5.2–§5.5 / §5.8–§5.10 type cleanups (including `AdminUser` / `TeamMember` local projection types that remain on files touched by the §1 PR), §6.1–§6.6 over-engineering judgment calls. The doc is otherwise still authoritative.
