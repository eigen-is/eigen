...
- **A big selection omits it.** The same records are serialized once as typed items, again into the SVG's `<metadata>`, and the whole SVG is then URI-encoded into an HTML attribute — a 500-shape select-all put ~1.1MB on `text/html`. Past `CLIPBOARD_SVG_MAX_ELEMENTS` (checked before the render, so a huge selection never pays for it) or `CLIPBOARD_SVG_MAX_BYTES` (the backstop for few-but-enormous elements), the flavor is skipped. The failure mode is deliberate and total: every eigen host still pastes losslessly from the typed items, and a foreign host gets the `text/plain` fallback instead of an image — the same thing it gets for a shape-only copy.
