...
steps:
  ...
  - bun --filter '!@apps/api' test --timeout 30000
  - bun --filter '@apps/api' test --parallel=4 --timeout 30000
```

The API suite runs with `--parallel=4` on CI, one isolated process per file on the runner's four cores, so no file inherits the heap, the resident Homes or the file order of the files before it; run as one process it stalled for seconds at a time (the roadmap row on the API suite has the measurements). The package suites still depend on file order and cannot take `--isolate` yet, so they stay sequential. The 30 s per-test timeout is CI-only: the runner is slower than a laptop, and bun's 5 s default turned every stall into a failure of whichever file was running. Locally the default stays, so a slow test is caught where it is written. Under `GITHUB_ACTIONS` the API preload prints `[memory] rss` every 5 s and bun prints per-test timings, so a stall shows in the log as a silent gap with the process size on either side.
