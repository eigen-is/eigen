...
**Keyboard:** Escape = close, ArrowLeft/ArrowRight = prev/next sibling, Space = close. They listen on the document, so they stand down while a layer is open above the overlay: `useDialogOpen(overlayRef)` (`packages/ui/src/hooks/use-dialog-open.ts`) asks whether any `role="dialog"` other than the overlay itself is mounted, which covers the runner's pickers and the `MessageView` header popover alike, and each of those closes itself on Escape. Space bows out when the keydown lands on a control inside the overlay, because there it is that control's own activation. The focus trap is the separate question: it stands down only for `runner.isDialogOpen`, since a body-portaled popover runs its own Tab cycle and the trap never sees those keys.
