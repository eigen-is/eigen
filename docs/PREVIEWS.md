...
> route with the contact cards themselves, rendered by `ContactDetailCard`, and an `.eml` on its own route
> with the message it holds. A mail part reaches the same
...
- `.eml` previews: the same shape again under `EML_FORMAT` — bumped on every change to `EmlPreview` **and on every DOMPurify upgrade**, because a cached body is html a previous sanitizer filtered
...
A mail part previews through the same renderers on its own bytes: `GET /mail/:ownerId/message/:id/attachment/:index/preview/text`, `.../preview/vcard` and `.../preview/eml` (`routes/mail.ts`) answer with the `TextPreviewResult`, the `VCardPreview` cards and the `EmlPreview` message the Drive routes answer with (all in `packages/lib/src/types/preview.ts`), two segments past the index so that a part named after a preview route cannot shadow it, so the same components render both. A forwarded message rides along as a `message/rfc822` part with its own bytes (`mail-parser/split.ts` flattens only an *inline* one into its parent), so the `.eml` route previews it like any other part. All three end in the one bytes-in entry point beside the cached ones — `getBytesTextPreview`, `getBytesVCardPreview` and `getBytesEmlPreview` (`preview-cache.ts`) — which own the decode and the Worker job; Drive reaches the same renderers through `getOrCacheText`, a part reaches them directly, because a part has no version stamp to key a cache on. `getBytesTextPreview` decodes with the charset the part declared, defaulting to UTF-8, and owns the text mode gate (`getBytesVCardPreview` has none — the routes run `assertVCardPreviewable` before it), and it gates on `getBytesTextPreviewMode`, never `getTextPreviewMode`: an eigen mime is the uploader's or the sender's word, so loose bytes render — and are labeled — as what their name says, rather than being drawn inside the A4, slide or canvas frame their mime claims. `assertVCardPreviewable` is the one `.vcf` gate both routes run (400 for a file the mime and name don't call a vCard, 413 past `IMPORT_MAX_BYTES`), `assertEmlPreviewable` the one `.eml` gate (400, and 413 past `EML_MAX_BYTES`); Drive runs them on the row before reading the bytes, mail on the parsed part, whose size is not known until then. The responses are `private, no-cache` with the ETag `readMailPart` builds, so a rewritten draft revalidates rather than serving what it had ([MAIL.md](MAIL.md)); a preview route hands it the renderer's format tag, so a payload or sanitizer fix (an `EML_FORMAT` bump) is answered with the new body rather than a 304 on a message whose own bytes never moved. `useMailTextPreview` / `useMailVCardPreview` (`packages/lib/src/core/mail/hooks/use-attachment-preview.ts`) read them, keyed per owner, message and part index; the cards ride the no-revival treaty (`mailVCardPreviewRoute`) for the same reason the Drive cards do — a bare `YYYY-MM-DD` birthday must not become a `Date` — and the message rides it through `mailEmlPreviewRoute` so its ISO `date` string stays the string the type declares.
...
Eigensheets, eigendoc, eigenslides and eigenvector preview generation runs in a one-shot Bun Worker so Yjs reconstruction, op replay, recalc, HTML rendering, and sanitization never block the API event loop (a `.vcf` and an `.eml` ride the same runner, carrying their own bytes instead of a captured document) ([DOCUMENT-TRANSFORMS.md](DOCUMENT-TRANSFORMS.md)). Every export rides the same runner through the same seam — see [EXPORT.md](EXPORT.md):
...
| `apps/api/src/routes/drive.ts`                                            | `/preview` + `/text-preview` + `/vcard-preview` + `/eml-preview` routes |
| `apps/api/src/routes/mail.ts`                                             | `/attachment/:index/preview/text` + `/preview/vcard` + `/preview/eml` for one mail part |
...
| `packages/lib/src/types/preview.ts`                                        | `TextPreviewResult` + `VCardPreview` + `EmlPreview`: the shapes the preview routes serve |
