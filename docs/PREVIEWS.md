...
FilePreview (fixed, z-[100])
  ...
  Footer   — Open (a Drive subject), then one button per FILE_ACTIONS row the subject
             ...
             Document, Import to Contacts), then "Download all (n)" for a batch
```
...
**The subject and its actions.** The overlay acts on a `FileSubject` (`packages/lib/src/types/file-subject.ts`): one file with a name, a mime, a size and the URLs to embed, download and thumbnail it, carrying the `DrivePath` behind it when a Drive item is what it holds. `subjectFromPath` (`packages/lib/src/core/file-subject.ts`) is the only place a Drive item becomes one, so no surface composes a preview route by hand. `openPreview(subject, siblings?, options?)` takes the siblings ← → pages through, and `{ batch: true }` marks them as a set to act on as a whole — which is what draws the "Download all (n)" row. A Drive listing hands over its whole folder for navigation and passes no option, so the overlay never offers to copy a folder onto itself; the attachment chips, whose siblings are one message's attachments, do pass it. What may be done with a subject is the registry `FILE_ACTIONS` (`packages/lib/src/core/file-actions.ts`), read through `fileActionsFor(subject, exclude?)` — each row's `applies` predicate reads the subject's own type and size, never which surface is asking. The overlay draws one footer button per row, excluding `quick-look`, being Quick Look itself. `useFileActionRunner` (`packages/ui/src/components/file-actions/use-file-action-runner.tsx`) performs a row and owns the single `SaveToDrivePicker` its host mounts through `runner.dialogs`: the footer's own Save to Drive row opens it on the previewed subject, and the batch row opens the same picker on the downloadable siblings through `runner.openPicker(subjects)`. Footer buttons disable while `runner.isPending`, and the overlay's focus trap hands focus over while `runner.isDialogOpen`. `SaveToDrivePicker` (`packages/ui/src/components/drive/save-to-drive-picker.tsx`) is the "where does this go" dialog for every surface that puts a file into Drive: a Drive subject is copied server-side so its bytes never travel, anything else falls back to a staggered browser download, and it renders above the preview overlay via the `abovePreview` prop on `DialogContent`.
