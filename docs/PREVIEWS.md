...
> screen-res WebP (max 2560px). Text, code, markdown, vCard and eigen-native files become a small HTML body
...
| `vcard`        | file bytes → transform Worker → one HTML block per contact card (`preview/vcard-render.ts`), the first 200 |
...
**vCard quick look.** A `.vcf` reads as contact cards, never as its raw text, so it rides the text mode with a server-rendered body. `getPreviewMode` (`packages/ui/src/components/preview-provider/preview-provider.tsx`) tests `isVCardFile` beside the text rule, while `getTextPreviewMode` returns `null` for the same files — which is what keeps a `.vcf` out of raw-text rendering, out of the inline editor (`isInlineEditable` is a separate list and `.vcf` is not on it) and out of the raw-read search path. `vcard` exists in `TextPreviewMode` as a served value only. `getTextPreview` branches on `isVCardFile` beside the collab split and rides the same `getOrCacheText` cache, so a `.vcf` body is generated once per file version like every other. The main thread refuses a file over `IMPORT_MAX_BYTES` with a "File too large to preview" notice without reading it; otherwise `runFileTransformToText` reads the bytes from the Mount and transfers them to the Worker, which renders them in `preview/vcard-render.ts` under the shared `preview` limits. The renderer decodes UTF-8 with `{ fatal: true }` — another encoding yields a "Could not read this file" notice rather than a throw — splits with `splitVCards`, transcodes and parses no more cards than an import would accept (`IMPORT_MAX_CARDS`), counts the cards the parser refuses rather than failing the whole file, and renders the first `VCARD_PREVIEW_MAX_CARDS` (200) as escaped semantic blocks: avatar, name, emails, phones, company, birthday, addresses, notes, and each card's `CATEGORIES` as badges. It shows the same fields as `ContactDetailCard` (`packages/ui/src/components/user/contact-detail-card.tsx`) — the composed ones (`formatContactAddress`, `formatContactRole`, `formatContactCompany`) come from `@workspace/lib/contacts/format` so both spell them once, and the two modules carry cross-reference comments so the rest stays in sync. `globals.css` styles the `.vcard-preview` markup beside the other preview-only classes, and `TextPreviewContent` renders the body in a plain `max-w-3xl` box rather than the prose wrapper (`WRAPPER_CLASS.vcard` is empty to match). Inline `PHOTO` bytes become `data:` URIs through `parsedCardToContact`; a `PHOTO;VALUE=uri` and a non-image media type are dropped rather than fetched, so an untrusted file can't make a viewer's browser call a URL it chose — and the assembled body still goes through `sanitizeExportHtml`, which keeps `data:` images and nothing else. The body ends on up to two counted lines, "and N more contacts" and "N contacts could not be read", spelled once in `packages/lib/src/vcard/preview-lines.ts`. The overlay header carries an extra button for a `.vcf` under the import ceiling — title **Import to Contacts** — which posts the previewed path to `/contacts/:ownerId/import-from-drive` ([CONTACTS.md](CONTACTS.md)). The drive detail hero wants three compact rows rather than the full column, so it parses the file in the browser instead: `VCardHero` (`packages/ui/src/components/drive/drive-preview.tsx`) reads it through `useVCardFile` (`packages/lib/src/core/contacts/hooks/use-transfer.ts`, keyed on the path's `updatedAt`, disabled over `IMPORT_MAX_BYTES`) and shows the first three cards as compact `UserAvatar` + name + first-email rows — the card's own name, never the address book's — falling back to the file icon over the ceiling, on a read error, and on a file with no readable card.
...
| `apps/api/src/lib/preview/vcard-render.ts`                                 | Worker-side `.vcf` body renderer (first 200 cards)                 |
| `packages/lib/src/core/contacts/hooks/use-transfer.ts`             | `useVCardFile()` — downloads and parses a `.vcf` for the drive hero |
