...
- `.eml` previews: the same shape again under `EML_FORMAT` — bumped on every change to `EmlPreview` **and on every DOMPurify upgrade**, because a cached body is html a previous sanitizer filtered; its route is the one that answers `private, no-cache` instead of a long `max-age`, so a bump reaches a browser that already holds a body
