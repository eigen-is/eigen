...
new version writes a **new file** instead of overwriting one — which is what lets the image preview and thumbnail responses use a long
`max-age`, since the URL carries the same `updatedAt` stamp. The JSON preview routes (text, vCard, `.eml`, `.ics`) revalidate instead: they answer `private, no-cache` with an ETag of the file's own etag plus the route's format tag (`isPreviewNotModified`, `lib/core/http.ts`), and a 304 when `If-None-Match` still matches.
