...
| `eigenvector`  | Yjs blobs → transform Worker → `sceneToSvg` → served as-is as `image/svg+xml` through the screen-preview image cache (sandbox CSP; no rasterizing, no HTML wrapper) |
