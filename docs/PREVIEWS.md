...
Everything in `mount.previewsDir` (`tmpDir/previews/`). The cache key carries the file version, so a new version writes a **new file** instead of overwriting one — which is what lets the image preview and thumbnail responses use a long `max-age`, since the URL carries the same `updatedAt` stamp. The JSON preview routes (text, vCard, `.eml`, `.ics`) revalidate instead, through `answerPreview` (`lib/core/http.ts`): the ETag is the file's own etag plus the route's format tag, an `If-None-Match` that still matches is answered with a 304 before anything generates, and only a generated body carries `private, no-cache` and the ETag. A failed generation (a 404, or an error the renderer raised) goes out with neither, because a browser would store that error with its validator and every later 304 would bring it back instead of retrying the generation. A stale body served while the current version regenerates is `no-store`.
...
4. The main thread writes the usual `{ body, mode }` cache envelope. One-shot Workers are terminated after
   ...
   `gracefulShutdown` (`src/server.ts`) closes the runner before mount teardown.
...
**Keyboard:** Escape = close, ArrowLeft/ArrowRight = prev/next sibling, Space = close. They listen on the document, so they stand down while a layer is open above the overlay, on two readings of the same fact. Presence is the steady state: `useDialogOpen(overlayRef)` (`packages/ui/src/hooks/use-dialog-open.ts`) asks whether any `role="dialog"` other than the overlay itself is mounted. **Where the key was pressed** is what decides the keydown in hand: a layer dismisses itself on the *capture* phase of the very keydown these document-level hotkeys hear on the bubble, and React has flushed it closed in between, so by then presence answers for the layer that just went. A keydown whose target sits inside a `role="dialog"`, `role="alertdialog"`, `role="menu"` or `role="listbox"` other than the overlay is that layer's — one rule for the runner's pickers, the `MessageView` header popover, every menu and a select's listbox, each of which takes focus when it opens. Space bows out again when the keydown lands on a control inside the overlay, because there it is that control's own activation — and the overlay registers Space with `preventDefault: false`, because the hotkey library prevents the default *before* the callback runs and a focused button would never press. The focus trap is the separate question: it stands down only for `runner.isDialogOpen`, since a body-portaled popover runs its own Tab cycle and the trap never sees those keys.
...
| `packages/lib/src/core/mail/hooks/use-attachment-preview.ts`              | `useMailTextPreview()` + `useMailVCardPreview()` + `useMailEmlPreview()` + `useMailIcsPreview()` hooks |
...
| `packages/lib/src/core/drive/hooks/reads.ts`                              | `useTextPreview()` + `useVCardPreview()` + `useEmlPreview()` + `useIcsPreview()` hooks |
