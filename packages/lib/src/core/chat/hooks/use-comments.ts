...
export function useUnresolvedCommentCount(ownerId: string, mountId: string, containerId: string) {
    return useQuery({
        ...
        queryFn: async () => {
            const response = await collabApi({ownerId})({mountId})({pathId: containerId}).comments['unresolved-count'].get();
...
export function invalidateComments(queryClient: QueryClient, ownerId: string, mountId: string, containerId: string): void {
    queryClient.invalidateQueries({queryKey: commentKeys.container(ownerId, mountId, containerId)});
}
