...
// The SVG flavor, resolved once. `svg` + `items` are the materialize-as-image inputs (docs/slides/
...
// flavor once (with the subtle guards baked in) so each app keeps only a short ladder that reads the
...
// readSvgClipboardWithItems so the svg flavor is derived from it without a second parse. Consumers
...
export function classifyPaste(cd: DataTransfer, opts?: { internalMarkerText?: string }): ClassifiedPaste {
    ...
    // the eigen wire — so an internal copy suppresses the eigen + svg flavors entirely (the caller then
