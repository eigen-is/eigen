    ...
    return useCallback(
        async (drafts: CardAttachmentDraft[]): Promise<ChatAttachment[]> => {
            ...
            for (const draft of drafts) {
                ...
                } else if (typeof draft === 'string' || isAttachmentReference(draft as ChatAttachment)) {
                    resolved.push(draft as ChatAttachment);
                } else if (isContainerType((draft as DrivePath).type)) {
                    resolved.push(toAttachmentReference(draft as DrivePath));
                } else {
                    if (!mediaFolderId) continue;
                    const copied = await copyToMediaFolder.mutateAsync({
                        paths: [draft as DrivePath],
                        mediaFolderId,
                    });
