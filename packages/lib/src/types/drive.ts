...
// Every image picker takes the whole image/* family, so no caller spells the prefix test itself.
...
// often application/octet-stream), so the .vcf extension counts on its own.
