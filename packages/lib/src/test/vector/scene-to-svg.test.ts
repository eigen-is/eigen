...
// The golden scene: each shape type, both roundness variants, a two-line text, the linear renders (a
...
export function buildGoldenScene(): VectorScene {
    return scene([
        ...
        // A PINNED elbow arrow (EP-U5): an unbound elbow whose middle is pinned to a vertical segment at
        // x-offset 40, forcing the route to detour through it. Placed inside the existing bounds so it only
        // APPENDS a fragment — every fragment above stays byte-identical. Exercises the multi-leg route +
        // the fixedSegments render path in the golden.
        {
            ...DEFAULT_ELEMENT_PROPS,
            id: 'ar7',
            type: 'arrow',
            ...
            endBinding: '',
            elbow: true,
            fixedSegments: '[{"start":[40,-20],"end":[40,80]}]',
            text: '',
            fontSize: 20,
            fontFamily: 'Excalifont',
            labelWidth: 0,
