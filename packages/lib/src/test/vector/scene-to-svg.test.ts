...
// an end head, a bound one with a triangle head and a masked two-line label, a bound sharp elbow arrow
...
export function buildGoldenScene(): VectorScene {
    return scene([
        ...
        // shaft is masked under the label. Bindings don't affect rendering (follow is a separate step).
        {
            ...DEFAULT_ELEMENT_PROPS,
            roughness: 1,
            id: 'ar2',
...
describe('sceneToSvg', () => {
    ...
    test('rich text rides into the foreignObject verbatim, its style attribute escaped', () => {
        ...
        // The inset — the user's 12px padding plus the 2px drawn border — shrinks the text area inside
        // the stored box rather than growing it.
        expect(svg).toContain('padding:14px;box-sizing:border-box');
    });

    test('a painted box draws its backdrop inside its own <g>, before the text', () => {
        const svg = sceneToSvg(scene([richtext({ id: 't', html: '<p>hi</p>', fill: solidFill('#ffcc00') })]));
        const group = svg.slice(svg.indexOf('<g transform='));
        expect(group.indexOf('<g stroke-linecap="round">')).toBeLessThan(group.indexOf('<foreignObject'));
        expect(group).toContain('#ffcc00');
    });

    test('rich text overflows its box instead of being clipped, and neutralises <p> margins', () => {
        const svg = sceneToSvg(scene([richtext({ id: 't', html: '<p>a long label</p>' })]));
        // A box stored at exactly its measured width wraps a hair wider in HTML shaping; the wrapped
        // line has to stay visible (foreignObject clips to its rect by default), as slides does.
        expect(svg).toContain('<foreignObject x="0" y="0" width="100" height="60" overflow="visible">');
        expect(svg).toContain('overflow:visible');
        // A standalone SVG has none of the app's CSS, so the UA's 1em <p> margins would push the text
        // out of the box the client measured. The reset rides along, scoped to our own wrapper.
        expect(svg).toContain('<style>.eigen-canvas-text p{margin:0}</style>');
    });
