...
export function installHappyDom(options: HappyDomOptions = {}): Window {
    ...
    // Every borrowed global is remembered as it was and PUT BACK in afterAll, never cleared: bun has an
    // Event and a CustomEvent of its own, and anything running after this teardown would find
    ...
    borrow('ResizeObserver', FakeResizeObserver);
    // Radix and the canvas hooks paint inside one rAF; running it inline keeps every assertion on the
    // settled result.
    borrow('requestAnimationFrame', (callback: () => void) => {
        callback();
        return 0;
    });
    borrow('cancelAnimationFrame', () => {});
