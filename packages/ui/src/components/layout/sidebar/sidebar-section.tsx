        ...
        <div className={cn('px-3 py-2', className)}>
            {title && (
                <div className={cn('flex items-center mb-2', condensed ? 'justify-center' : 'justify-between')}>
                    {!condensed && <h3 className="text-sm font-medium text-muted-foreground">{title}</h3>}
                    {condensed && <span className="text-sm font-medium text-muted-foreground">{title[0]}</span>}
                </div>
            )}
