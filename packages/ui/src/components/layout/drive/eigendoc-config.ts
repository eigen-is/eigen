...
// `?chat=` / `?card=` deep-link params so they can't drift (only stickies reads `card` for now).
export function eigenDocEditorValidateSearch(search: Record<string, unknown>): { chat?: string; card?: string } {
    return {
