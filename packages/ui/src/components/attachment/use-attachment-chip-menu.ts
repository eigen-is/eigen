...
// The long-press handlers plus the two this hook adds, so the shape stays spelled once.
export type ChipMenuHandlers<H> = ReturnType<ReturnType<typeof useLongPress<H>>['bind']> & {
...
export type AttachmentChipMenu<H, T> = {
    ...
    bind: (host: H) => ChipMenuHandlers<H>;
};
...
export function useAttachmentChipMenu<H, T>(toMenuItem: ToMenuItem<H, T>): AttachmentChipMenu<H, T> {
    ...
    const handleLongPress = useCallback(
        (host: H, x: number, y: number) => {
            ...
            if (item !== undefined) openAt(item, x, y);
        },
        [openAt, toMenuItem],
    );
    ...
    const bind = useCallback(
        (host: H) => ({
            onContextMenu: (e: React.MouseEvent) => {
                ...
                // Leave links and selected text to the browser's native copy menu. A chip is an
                // anchor too, and it has its own rows to offer.
