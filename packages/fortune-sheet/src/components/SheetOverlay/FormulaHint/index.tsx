...
export const FormulaHint: React.FC<React.HTMLAttributes<HTMLDivElement>> = (props) => {
    ...
    return (
        ...
        >
            ...
            <div
                className="absolute top-0 right-6 text-base cursor-pointer text-muted-foreground hover:text-foreground"
                title="Collapse"
            >
            ...
            <div className="block border-y border-[#ebebeb] bg-[#f5f5f5] px-2.5 py-0.5 text-sm">
                <div className="w-[250px] break-words">
                    <span className="luckysheet-arguments-help-function-name">{fn.n}</span>
                    ...
                    <span className="luckysheet-arguments-parameter-holder">
                        {fn.p.map((param, i) => {
                            ...
                            return (
                                <span className="luckysheet-arguments-help-parameter" dir="auto" key={name}>
                                    {name}
                                    {i !== fn.p.length - 1 && ', '}
                                </span>
                            );
            ...
            <div className="max-h-[300px] overflow-y-auto">
                <div className="mt-1.5">
                    <div className="px-2.5 py-px text-[#666]">{formulaMore.helpExample}</div>
                    <div className="px-2.5 py-px text-sm">
                        <span className="luckysheet-arguments-help-function-name">{fn.n}</span>
                        <span className="luckysheet-arguments-paren">(</span>
                        <span className="luckysheet-arguments-parameter-holder">
                            {fn.p.map((param, i) => (
                                <span key={param.name} className="luckysheet-arguments-help-parameter" dir="auto">
                                    {param.example}
                                    {i !== fn.p.length - 1 && ', '}
                                </span>
                            ))}
                ...
                <div className="luckysheet-formula-help-content-detail">
                    <div className="my-1.5 text-[#222]">
                        <div className="px-2.5 py-px text-[#666]">{formulaMore.helpAbstract}</div>
                        <span className="px-2.5 py-px inline-block break-words">{fn.d}</span>
                    </div>
                </div>
                    ...
                    {fn.p.map((param) => (
                        <div className="my-1.5 text-[#222]" key={param.name}>
                            <div className="px-2.5 py-px text-[#666]">
                                ...
                                {param.require === 'o' && (
                                    <span className="text-sm text-gray-500">-[{formulaMore.allowOptionText}]</span>
                                )}
                            </div>
                            <span className="px-2.5 py-px inline-block break-words">{param.detail}</span>
                        </div>
                    ))}
                </div>
            </div>
