...
describe('drawCellIndicator', () => {
    test('a left-corner mark runs right and down from its anchor', () => {
        ...
        drawCellIndicator(canvas.renderCtx, 'left', { x: 10, y: 40, size: COMMENT_INDICATOR_SIZE }, '#487f1e');
        expect(canvas.points).toEqual([
            [10, 40],
            [10 + COMMENT_INDICATOR_SIZE, 40],
            [10, 40 + COMMENT_INDICATOR_SIZE],
        ]);
    ...
    test('a right-corner mark mirrors it, same legs', () => {
        ...
        expect(canvas.points).toEqual([
            [100, 40],
            [100 - COMMENT_INDICATOR_SIZE, 40],
            [100, 40 + COMMENT_INDICATOR_SIZE],
        ]);
    });
    ...
    test('both corners draw legs of exactly one size', () => {
        for (const corner of ['left', 'right'] as const) {
            const canvas = recordingCanvas();
            drawCellIndicator(canvas.renderCtx, corner, { x: 50, y: 0, size: COMMENT_INDICATOR_SIZE }, '#000000');
            const [origin, along, down] = canvas.points;
            expect(Math.abs(along[0] - origin[0])).toBe(COMMENT_INDICATOR_SIZE);
            expect(down[1] - origin[1]).toBe(COMMENT_INDICATOR_SIZE);
        }
    });
});
