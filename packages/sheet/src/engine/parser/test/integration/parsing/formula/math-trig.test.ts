...
describe('.parse() math-trig formulas', () => {
    ...
    it('AGGREGATE', () => {
        parser!.on('callRangeValue', (_a, _b, done) => {
            done([[1, 2, 3]]);
        });
    ...
    it('FLOOR', () => {
        ...
        expect(parser!.parse('FLOOR(1)')).toMatchObject({ error: null, result: 0 });
    ...
    it('SERIESSUM', () => {
        ...
        expect(parser!.parse('SERIESSUM(PI() / 4, 0, 2, SERIESSUM_ARR)')).toMatchObject({
            error: '#ERROR!',
            result: null,
        });
    });
