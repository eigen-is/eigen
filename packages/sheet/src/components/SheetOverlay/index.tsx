...
export const SheetOverlay: React.FC = () => {
    ...
    // The body overlays no longer scroll natively with cellArea; they live in a
    // position:sticky layer pinned to the scrollport origin and track the single
    // scroll offset on the bus by translating their content on both axes — the
    // exact ColumnHeader pattern. notifyScrollListeners runs synchronously, so this
    // stays locked to the rAF canvas redraw for both user and programmatic scroll
    // and can no longer drift a frame the way native scrolling did. Applied once on
    // mount for the initial offset.
    useEffect(() => {
        const applyOffset = () => {
            if (overlayLayerRef.current) {
                overlayLayerRef.current.style.transform = `translate(${-refs.globalCache.scrollLeft}px, ${-refs.globalCache.scrollTop}px)`;
            }
        };
        applyOffset();
            ...
            <div className="fortune-row-body">
                ...
                >
                    <div className="fortune-cell-overlay-layer">
                        <div ref={overlayLayerRef} style={{ position: 'absolute', inset: 0 }}>
