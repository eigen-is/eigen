...
describe('Sharing Restricted', () => {
    ...
    describe('default behavior (unrestricted)', () => {
        ...
        beforeAll(async () => {
            const folder = await drivePost(ctx.alice.user.sessionToken, ctx.alice.user.id, aliceMountId,
                `folder/${aliceRootId}`, {folderName: 'Unrestricted Folder'});
            folderId = folder.id;
        ...
        test('sharingRestricted defaults to false', async () => {
            const path = await driveGet(ctx.alice.user.sessionToken, ctx.alice.user.id, aliceMountId, `path/${folderId}`);
    ...
    describe('owner restricts sharing', () => {
        ...
        beforeAll(async () => {
            const folder = await drivePost(ctx.alice.user.sessionToken, ctx.alice.user.id, aliceMountId,
                `folder/${aliceRootId}`, {folderName: 'Restricted Folder'});
            folderId = folder.id;
        ...
        test('editor is blocked from modifying ACL', async () => {
            ...
            // ACL unchanged
            const path = await driveGet(ctx.alice.user.sessionToken, ctx.alice.user.id, aliceMountId, `path/${folderId}`);
            const charlieEntry = path.acl?.find((e: any) => e.id === CHARLIE_EMAIL);
        ...
        test('editor can still edit content (write is not affected)', async () => {
            const subfolder = await drivePost(ctx.bob.user.sessionToken, ctx.alice.user.id, aliceMountId,
                `folder/${folderId}`, {folderName: 'Bob Can Still Write'});
    ...
    describe('viewer cannot modify ACL regardless of restriction', () => {
        ...
        beforeAll(async () => {
            const folder = await drivePost(ctx.alice.user.sessionToken, ctx.alice.user.id, aliceMountId,
                `folder/${aliceRootId}`, {folderName: 'Viewer Test Folder'});
            folderId = folder.id;
    ...
    describe('inherited write access + restricted child', () => {
        ...
        beforeAll(async () => {
            const parent = await drivePost(ctx.alice.user.sessionToken, ctx.alice.user.id, aliceMountId,
                `folder/${aliceRootId}`, {folderName: 'Reshare Inherited Parent'});
            parentId = parent.id;
            ...
            const child = await drivePost(ctx.alice.user.sessionToken, ctx.alice.user.id, aliceMountId,
                `folder/${parentId}`, {folderName: 'Restricted Child'});
            childId = child.id;

            await drivePut(ctx.alice.user.sessionToken, ctx.alice.user.id, aliceMountId,
                `path/${childId}/acl`, {acl: [], sharingRestricted: true});
        });
        ...
        test('Bob can still create content in restricted child', async () => {
            const subfolder = await drivePost(ctx.bob.user.sessionToken, ctx.alice.user.id, aliceMountId,
                `folder/${childId}`, {folderName: 'Content OK'});
    ...
    describe('sharingRestricted propagates to shared_paths', () => {
        ...
        beforeAll(async () => {
            const folder = await drivePost(ctx.alice.user.sessionToken, ctx.alice.user.id, aliceMountId,
                `folder/${aliceRootId}`, {folderName: 'Propagation Test'});
            folderId = folder.id;
        ...
        test('Bob sees sharingRestricted in shared-with-me', async () => {
            ...
            const shared = data.find(item => item.id === folderId);
    ...
    describe('team-owned paths', () => {
        ...
        beforeAll(async () => {
            ...
            const folder = await drivePost(ctx.alice.user.sessionToken, teamOwner, teamMountId,
                `folder/${teamRootId}`, {folderName: 'Team Restricted Folder'});
            folderId = folder.id;

            await drivePut(ctx.alice.user.sessionToken, teamOwner, teamMountId,
                `path/${folderId}/acl`, {
