...
describe('Serve-file caching', () => {
    ...
    test('preview and thumbnail responses are private with a max-age', async () => {
        ...
        let thumb: Response | undefined;
        for (let i = 0; i < 20; i++) {
            thumb = await authedRequest(token, `/drive/${ownerId}/${mountId}/thumb/${uploaded.id}.webp`);
            if (thumb.status === 200) break;
            await new Promise((r) => setTimeout(r, 250));
        }
        expect(thumb!.status).toBe(200);
        expect(thumb!.headers.get('cache-control')).toStartWith('private, max-age=');
    });
});
