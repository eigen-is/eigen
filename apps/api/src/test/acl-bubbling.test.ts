...
describe('ACL Bubbling', () => {
    ...
    describe('standalone chat (not inside a container)', () => {
        ...
        beforeAll(async () => {
            const chat = await drivePost(ctx.alice.user.sessionToken, ctx.alice.user.id, aliceMountId,
                `folder/${aliceRootId}/chat`, {fileName: 'standalone-room'});
            chatId = chat.id;

            // Share chat with Bob so he can write
            await drivePut(ctx.alice.user.sessionToken, ctx.alice.user.id, aliceMountId,
                `path/${chatId}/acl`, {acl: [{id: BOB_EMAIL, read: true, write: true}]});
        });

        test('owner invite sets ACL on the chat itself', async () => {
            const res = await invite(ctx.alice.user.sessionToken, ctx.alice.user.id, aliceMountId, chatId, CHARLIE_EMAIL);
        ...
        test('editor can invite to standalone chat', async () => {
            const res = await invite(ctx.bob.user.sessionToken, ctx.alice.user.id, aliceMountId, chatId, 'newuser@test.eigen.is');
            expect(res.status).toBe(200);
            const body = await res.json() as any;
            expect(body.alreadyHasAccess).toBe(false);
            expect(body.targetPathId).toBe(chatId);
    ...
    describe('embedded chat inside eigendoc', () => {
        ...
        beforeAll(async () => {
            const doc = await drivePost(ctx.alice.user.sessionToken, ctx.alice.user.id, aliceMountId,
                `folder/${aliceRootId}/doc`, {fileName: 'acl-bubble-doc'});
            docId = doc.id;

            // Create chat inside the doc
            const chat = await drivePost(ctx.alice.user.sessionToken, ctx.alice.user.id, aliceMountId,
                `folder/${docId}/chat`, {fileName: 'discussion'});
            chatId = chat.id;
        ...
        test('invite via embedded chat sets ACL on the container doc', async () => {
            ...
            const docPath = await driveGet(ctx.alice.user.sessionToken, ctx.alice.user.id, aliceMountId, `path/${docId}`);
            expect(docPath.acl).toContainEqual({id: CHARLIE_EMAIL, read: true, write: true});

            const chatPath = await driveGet(ctx.alice.user.sessionToken, ctx.alice.user.id, aliceMountId, `path/${chatId}`);
            const chatCharlie = chatPath.acl?.find((e: any) => e.id === CHARLIE_EMAIL);
            expect(chatCharlie).toBeUndefined();
        });

        test('editor invite via embedded chat also sets ACL on container doc', async () => {
            const res = await invite(ctx.bob.user.sessionToken, ctx.alice.user.id, aliceMountId, chatId, 'editor-invite@test.eigen.is');
            expect(res.status).toBe(200);
            const body = await res.json() as any;
            expect(body.targetPathId).toBe(docId);
    ...
    describe('chat inside folder inside eigendoc (nested)', () => {
        ...
        beforeAll(async () => {
            const doc = await drivePost(ctx.alice.user.sessionToken, ctx.alice.user.id, aliceMountId,
                `folder/${aliceRootId}/doc`, {fileName: 'nested-container-doc'});
            docId = doc.id;
            ...
            const contents = await driveGet(ctx.alice.user.sessionToken, ctx.alice.user.id, aliceMountId, `folder/${docId}`);
            const chatFolder = contents.find((p: any) => p.name === 'chat');

            const chat = await drivePost(ctx.alice.user.sessionToken, ctx.alice.user.id, aliceMountId,
                `folder/${chatFolder.id}/chat`, {fileName: 'nested-discussion'});
            chatId = chat.id;
        });
    ...
    describe('validation and permissions', () => {
        ...
        beforeAll(async () => {
            const chat = await drivePost(ctx.alice.user.sessionToken, ctx.alice.user.id, aliceMountId,
                `folder/${aliceRootId}/chat`, {fileName: 'perm-test-room'});
            chatId = chat.id;
        });

        test('invalid email returns 400', async () => {
            const res = await invite(ctx.alice.user.sessionToken, ctx.alice.user.id, aliceMountId, chatId, 'not-an-email');
        ...
        test('viewer on chat gets 403', async () => {
            ...
            await drivePut(ctx.alice.user.sessionToken, ctx.alice.user.id, aliceMountId,
                `path/${chatId}/acl`, {acl: [{id: BOB_EMAIL, read: true, write: false}]});
        ...
        test('email is case-insensitive', async () => {
            const res1 = await invite(ctx.alice.user.sessionToken, ctx.alice.user.id, aliceMountId, chatId, 'CaseTest@Example.COM');
            expect(res1.status).toBe(200);

            // Same email different case is a duplicate
            const res2 = await invite(ctx.alice.user.sessionToken, ctx.alice.user.id, aliceMountId, chatId, 'casetest@example.com');
        ...
        test('self-invite is allowed', async () => {
            const res = await invite(ctx.alice.user.sessionToken, ctx.alice.user.id, aliceMountId, chatId, ctx.alice.user.email);
            expect(res.status).toBe(200);
        });
    });
    ...
    describe('chat write but no container write', () => {
        ...
        beforeAll(async () => {
            const doc = await drivePost(ctx.alice.user.sessionToken, ctx.alice.user.id, aliceMountId,
                `folder/${aliceRootId}/doc`, {fileName: 'no-doc-write-test'});

            const chat = await drivePost(ctx.alice.user.sessionToken, ctx.alice.user.id, aliceMountId,
                `folder/${doc.id}/chat`, {fileName: 'restricted-chat'});
            chatId = chat.id;
    ...
    describe('sharingRestricted on container', () => {
        ...
        beforeAll(async () => {
            const doc = await drivePost(ctx.alice.user.sessionToken, ctx.alice.user.id, aliceMountId,
                `folder/${aliceRootId}/doc`, {fileName: 'restricted-container-doc'});
            docId = doc.id;

            const chat = await drivePost(ctx.alice.user.sessionToken, ctx.alice.user.id, aliceMountId,
                `folder/${docId}/chat`, {fileName: 'restricted-container-chat'});
            chatId = chat.id;
        ...
        test('editor blocked from inviting when container is sharing-restricted', async () => {
            ...
            // ACL unchanged
            const docPath = await driveGet(ctx.alice.user.sessionToken, ctx.alice.user.id, aliceMountId, `path/${docId}`);
            const charlieEntry = docPath.acl?.find((e: any) => e.id === CHARLIE_EMAIL);
        ...
        test('editor can invite after owner removes restriction', async () => {
            ...
            const res = await invite(ctx.bob.user.sessionToken, ctx.alice.user.id, aliceMountId, chatId, 'unrestricted-invite@test.eigen.is');
            expect(res.status).toBe(200);
        });
    });
    ...
    describe('nonexistent chat', () => {
        test('invite to nonexistent chat returns 404', async () => {
            const res = await invite(ctx.alice.user.sessionToken, ctx.alice.user.id, aliceMountId, 'nonexistent-id', BOB_EMAIL);
