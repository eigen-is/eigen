// Merging Eigen-owned edits back into a stored card and building a fresh one (src/lib/vcard/serialize.ts):
// every line the app does not own keeps its source bytes, and every edited value is escaped at the write
// seam. The AST, projection parse and 4.0 -> 3.0 transcode are tested in vcard.test.ts.
