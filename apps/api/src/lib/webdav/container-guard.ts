...
// SLIDES, SHEETS, CHAT — anything in EIGEN_DOCUMENT_TYPES) along the
// breadcrumb, or null. Plain folders never match. Used by WebDAV to gate
// writes that would land inside a managed container — those resources have
// app-managed internal state (data.db, media/) that the drive layer owns,
// so client-driven PUT/MKCOL/DELETE there would corrupt or orphan rows.
//
// Memory note 2026-04-30: containers are read-only, not opaque — reads
// (PROPFIND, GET, COPY-out) are allowed; only writes are blocked.
//
// includeSelf:
//   false — "is this resource inside a container?" (DELETE, MOVE source,
//           overwrite-PUT existing): the resource itself being a container
//           is fine; we care about its ancestors.
