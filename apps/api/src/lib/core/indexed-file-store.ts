...
import { normalizeResourceUri, sanitizeResourceUri } from './blob-store';
import { isEnoent, type LocalFilesystem } from './local-filesystem';

// The domain-neutral half of a file+index store, with no SQL. Calendar's half until it moves onto core/blob-store.ts.
...
export function uriKeyOf(uri: string): string {
    return normalizeResourceUri(uri).toLowerCase();
}
