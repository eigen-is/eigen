...
import { mailboxKeys, useMailboxes, findCorrectMailboxPath } from './use-mailboxes';
  ...
  return useQuery({
    ...
    queryFn: async () => {
      try {
        ...
        // Handle special case for Archive mailbox
        const isArchive = finalMailboxPath === 'Archive' || finalMailboxPath === '.Archive';
        
        // The API doesn't have a direct endpoint to list emails in a mailbox,
        // but we can get mailbox info which includes messages
        // @ts-ignore - Dynamic property access
        const response = await mailApi.mailbox[finalMailboxPath].get();
        console.log(`API response for ${finalMailboxPath}:`, response);
        
        // If there's no data, return an empty array
        if (!response || !response.data) {
          console.log(`No data found for ${finalMailboxPath}`);
          return [];
        }
        ...
        if (Array.isArray(response.data)) {
          ...
          if (response.data.length === 0) {
            console.log(`No emails found in ${finalMailboxPath}`);
            return [];
          }
          
          // For Archive mailbox, the data might be nested differently
          if (isArchive) {
            // Try to extract data from the first item if it's an object
            const firstItem = response.data[0];
            if (firstItem && typeof firstItem === 'object') {
              console.log(`First item keys:`, Object.keys(firstItem));
              
              // Check if the first item has html content (might be a single email)
              if (firstItem.html || firstItem.text) {
                console.log(`Found single email in Archive`);
                return [transformEmailData(firstItem)];
              }
              
              // Check if there's a nested array we can use
              for (const key of Object.keys(firstItem)) {
                if (Array.isArray(firstItem[key])) {
                  console.log(`Found nested array in ${key} with ${firstItem[key].length} items`);
                  return firstItem[key].map(transformEmailData);
                }
              }
              
              // If we have attachments, this is likely a single email
              if (firstItem.attachments) {
                console.log(`Found email with attachments in Archive`);
                return [transformEmailData(firstItem)];
              }
              
              // Last resort: treat the first item as an email
              return [transformEmailData(firstItem)];
            }
          }
          
          return response.data.map(transformEmailData);
        }
        
        // Check if response.data is a single object (might be a single email)
        if (response.data && typeof response.data === 'object' && !Array.isArray(response.data)) {
          console.log(`Response.data is a single object`);
          
          // Check if it has email properties
          if (response.data.subject || response.data.from || response.data.to || response.data.html || response.data.text) {
            console.log(`Found single email object`);
            return [transformEmailData(response.data)];
          }
          
          // Check if there's a messages property
          if (response.data.messages && Array.isArray(response.data.messages)) {
            console.log(`Found messages array with ${response.data.messages.length} items`);
            return response.data.messages.map(transformEmailData);
          }
          ...
          for (const key in response.data) {
            if (Array.isArray(response.data[key])) {
              ...
              return response.data[key].map(transformEmailData);
            }
          }
        }
        
        console.error(`Could not find any email array in the response for ${finalMailboxPath}`);
        return [];
      } catch (error) {
        console.error(`Error fetching emails for mailbox ${finalMailboxPath}:`, error);
        return [];
      }
    },
    enabled: !!finalMailboxPath,
