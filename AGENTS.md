...
| **Document transforms** | `apps/api/src/lib/document/transform/`     | Bounded runner + one-shot Bun Workers for CPU-heavy document transforms (every eigensheets/eigendoc/eigenslides preview, HTML/XLSX/PDF exports and xlsx import/convert today; DOCX in a later phase). One main-thread seam (`run-transform.ts`), typed closed protocol, never a main-thread fallback; media prep and the import commit stay on the main thread. See [PREVIEWS.md](docs/PREVIEWS.md) + [EXPORT.md](docs/EXPORT.md) + [PROPOSAL_DOCUMENT_TRANSFORM_WORKERS.md](docs/PROPOSAL_DOCUMENT_TRANSFORM_WORKERS.md) |
