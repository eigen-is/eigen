...
| **Document transforms** | `apps/api/src/lib/document/transform/`     | Bounded runner + one-shot Bun Workers for CPU-heavy document transforms (eigensheets preview + HTML/XLSX/PDF exports today; imports and doc/slides in later phases). One main-thread seam (`run-transform.ts`), typed closed protocol, never a main-thread fallback. See [PREVIEWS.md](docs/PREVIEWS.md) + [EXPORT.md](docs/EXPORT.md) + [PROPOSAL_DOCUMENT_TRANSFORM_WORKERS.md](docs/PROPOSAL_DOCUMENT_TRANSFORM_WORKERS.md) |
