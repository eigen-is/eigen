...
- **Always run `bun run check`** after changes — lint, typecheck, the `getHome()` import guard, the shared-primitives
  index check, the docs link check, the ratcheting standards gate (`scripts/check-standards.ts` — see
  [CODE-STANDARDS.md § Standards Gates](docs/CODE-STANDARDS.md#standards-gates)), and every workspace's tests. When
  multiple agents run in parallel, only the main agent should run check — concurrent runs cause deadlocks
...
- **The backend imports lib through React-free subpaths, never `core/` domain barrels** — every
  ...
  `vector`(`/*`), `background`, `docs/eigendoc`, and the React-free leaf modules (`date`, `format`, `html`). For a React-free
...
| **Collab storage**    | `apps/api/src/lib/collab/`                   | Yjs updates/snapshots in `data.db` BLOBs; zstd-compressed at the storage seam (`blob-codec.ts`), BC via zstd magic-byte sniff; live WS sync unaffected. Awareness frames are validated before apply in `CollabDocument.handleMessage` (at most 8 client ids and 16 KiB per state per connection, `user.userId` must be the session user, a client id is owned by the connection that declared it). The WS route sends a loading heartbeat (`loading-heartbeat.ts`) until sync-step-1 so a slow cold load never trips y-websocket's 30s silence timeout, and docs linger 60s after the last unsubscribe (`CollabDocument.scheduleClose`) so reconnects reattach instead of re-paying the load |
...
| **CardDAV**           | `apps/api/src/lib/carddav/`                  | RFC 6352 server at `/dav/addressbooks/:ownerId/…`; contacts as vCard files under `eigen.contacts/cards/` with `contacts.db` an index over those cards **plus** authoritative sync/label metadata (labels, book row, tombstones, journals — not re-derivable; a rebuild rotates `syncGen`); mirrors CalDAV layer. The DAV store seam, reconcile/rebuild passes, labels and avatars live in sibling `contacts/*.ts` modules (plain functions over the contacts — `Contacts` stays the facade, the Mount pattern). See [CONTACTS.md](docs/CONTACTS.md) |
...
| **Backup / restore**  | `apps/api/src/lib/backup/` + `apps/api/src/routes/backup.ts` | Per-home backup and restore, admin-only under `/admin/backup/*` (the `settings.ts` carve-out: server-wide surface, so no `:ownerId` as the second path segment — the two safety-copy routes carry one further down — and `requireAdmin` in every handler). One primitive, `snapshotHome`, writes a complete storage-independent copy of one home (`VACUUM INTO` per database, containers freshest-first, S3 objects materialized, `versions/` + `.trash/` + `thumbs/` in — a thumbnail is written once at upload and never regenerated — `tmp/` and `staging/` out, empty directories carried so a restored Maildir keeps the folders its watchers need) into a folder; `archive.ts` packs it as `.tar.zst` through our own streaming pax tar writer — `Bun.Archive.write` writes lazy `Bun.file` entries as EMPTY and buffers the whole archive, so never "simplify" back to it; reading stays on `Bun.Archive`. `verify.ts` judges a folder in three stages (per-entry sha256, `quick_check` on every Eigen-owned db, Yjs decode on a deterministic sample). `restore.ts` replaces a home and deletes nothing: safety copies `{id}.pre-restore-{ts}` / `{id}.failed-restore-{ts}`, fresh storage keys on remote mounts (so a safety copy is complete on S3 too), database + schema-version checks BEFORE any `users3.db` write, boot recovery off a `restoring.json` marker. `jobs.ts` is an in-memory job map (one job per home, `backup:job-updated` SSE as the poke, the backups folder is the durable record). `markHomeRestoring` makes `getHome` refuse the home on every request-resolved surface (HTTP, SSE, collab WS, CalDAV, CardDAV, WebDAV) while its folder is replaced, and its collab sockets close with `COLLAB_HOME_REPLACED_CLOSE` (1012) so tabs reload instead of syncing stale state; IMAP (Dovecot, out of process) keeps serving the Maildir, which is a documented window. `lib/backup/*` never imports `getHome` — the route resolves the home and hands it to the job. See [BACKUP.md](docs/BACKUP.md) |
...
| **Yjs utilities**  | `packages/lib/src/core/collab/yjs-utils.ts`           | `restoreYjsDoc` — server-side only (used by `CollabDocument.applySnapshotState` during version restore to replace the live Y.Doc's declared roots inside one transaction, so connected editors converge with no reload). Handles Y.Map, Y.Array, Y.Text, and Y.XmlFragment/Tiptap, with arbitrary nesting. Also the home of `getItemMapRoot` / `getIdArrayRoot` / `getIdArray` — the shared typed root/id-array accessors stickies reads its roots and nested id lists through instead of casting (the FE hooks and the BE `document/stickies.ts` reader alike). The canvas engine needs none of them: its roots are flat maps of scalar-only element records |
| **Collab lifecycle** | `packages/lib/src/core/collab/hooks/use-collab-doc.ts` | `useCollabDoc` owns the Y.Doc + WebsocketProvider + UndoManager for every collab editor (docs/slides/stickies/vector; sheets layers its op-log on top). Exposes live `synced` (presence + seed-on-sync), `offline` (socket down after first load and not a storage outage) AND a latched `loaded` — **gate the loading screen on `loaded`, never `synced`**, so a WS blip doesn't unmount the editor. `offline` waits out a ~1.5 s grace window so a reconnect within a blip never flashes the indicator, and it feeds the shared `DocumentShareCluster` beside `storageUnavailable`: one red wifi-off icon, whose popover either promises a sync when back online or explains that storage is retrying. Edits live only in the tab until the server acknowledges them, so the hook also exposes `unsyncedEdits` — armed by an update applied while the socket is down, and by the `disconnected` event when this tab pushed updates since the last handshake (a silently dead socket keeps `wsconnected` true until y-websocket's 30 s silence check fires), cleared by the next sync. Every editor renders `<UnsyncedEditsGuard active={unsyncedEdits} />` (`packages/ui/src/components/layout/app/unsynced-edits-guard.tsx`), which arms the router's own `beforeunload` for reloads and blocks in-app navigation with a `ConfirmDialog`. Repairs/technical fixups write under non-null origin sentinels (`NORMALIZE_ORIGIN`, vector's `UNTRACKED_ORIGIN`) that escape the default `{null}` trackedOrigins. `storageUnavailable` is set by a `COLLAB_STORAGE_UNAVAILABLE_CLOSE` (1013, reason `storage-unavailable`) close the WS route sends when the document's storage 503s; the hook pauses the provider and reconnects after `STORAGE_RETRY_MS` because y-websocket stops backing off once a socket has opened. A `COLLAB_HOME_REPLACED_CLOSE` (1012, reason `home-replaced`) close — sent by `closeCollabConnectionsForHome` when a backup restore replaces the home — is the opposite: the hook disconnects and reloads the page, because a tab that reconnected would sync the document it still holds in memory back over the restored copy; the hook disarms the unsynced-edits guard before reloading, because those edits belong to a home that no longer exists (see [BACKUP.md](docs/BACKUP.md)). The five editors render the shared `CollabLoadingState` (slow notice after 10 s, unavailable notice on the flag), and the routes render `EigenDocRouteStatus` (spinner or `ErrorState`) while `useEigenDocEditorRoute` has no `docInfo` — `useCollabDocumentInfo` maps only 401/403 to `canRead: false` and throws the rest, so an infrastructure error never renders RequestAccessView. See [CANVAS.md](docs/CANVAS.md) |
| **Canvas engine** | `packages/lib/src/vector/` + `packages/ui/src/components/vector/` | One engine for free-canvas documents, mounted by `apps/vector`. lib is React-free: the element model (every stored field a scalar, arrays as JSON), the kind registry (`ELEMENT_KINDS` + `capabilitiesOf(el)` — one file per kind answers fields, capabilities, geometry, outline, render and search text), the `core ⊕ disc` outline math the renderer AND the arrow docking share, `elementLayer`/`sceneLayers`, the reader and `sceneToSvg`. ui is the host: `CanvasEditor` (viewport `infinite` | `frame`, frame ops on the doc hook, per-element comments via `commentCardIds`, ⌘F through `searchScene`, in-place rich text, the capability-gated properties panel, the typed `elements` clipboard item) plus `ELEMENT_KIND_UI`, the React half of the registry. Every element renders as one absolutely positioned layer div. See [CANVAS.md](docs/CANVAS.md) + [CLIPBOARD.md](docs/CLIPBOARD.md) |
| **Canvas editors** | `packages/ui/src/components/vector/` + `apps/vector/` + `apps/slides/` | One engine, two apps: `CanvasEditor` on the infinite canvas (drawings) and in frame mode (a deck of 1920×1080 slide frames — `apps/slides` is the shell: rail, present mode, background scope, counter). They share one keymap, layered-Escape stack, and primitives: `ObjectTransform` + `snapBox`, `TransformSection`/`AlignSection`/`ZOrderButtons`, `marqueeMode`/`marqueeHits`, `snap.ts` (screen-vs-scene threshold, `centerOnly` for rotated), `computeArrange`, `stopCapturing` sealing. Typed clipboard items carry geometry + typography as first-class fields; the flavor policy avoids double-paste. See [CANVAS.md](docs/CANVAS.md) + [CLIPBOARD.md](docs/CLIPBOARD.md) |
