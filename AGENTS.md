...
| **Document transforms** | `apps/api/src/lib/document/transform/`     | Bounded runner + one-shot Bun Workers for CPU-heavy document transforms (eigensheets preview today; exports/imports and doc/slides in later phases). Typed closed protocol, never a main-thread fallback. See [PREVIEWS.md](docs/PREVIEWS.md) + [PROPOSAL_DOCUMENT_TRANSFORM_WORKERS.md](docs/PROPOSAL_DOCUMENT_TRANSFORM_WORKERS.md) |
