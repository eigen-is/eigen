...
| **Search**         | `packages/lib/src/core/search/` + `apps/api/src/lib/search/` | FTS5-backed. Backend: `SearchIndex` per Home with `itemIds`/`buckets` filters; Mail uses filter-first via `mail.db` for `from`/`to`. Frontend: `useSearch` hook. SSE handlers invalidate `searchKeys.owner` on mail mutations. See [PROPOSAL_SEARCH.md](docs/PROPOSAL_SEARCH.md) |
