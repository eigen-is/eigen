...
| **New-chat wizard** | `packages/lib/src/core/chat/hooks/use-chat.ts` + `packages/ui/src/components/layout/chat/chat-create-wizard.tsx` | `ChatCreateWizard({open,onOpenChange,initialPeople?,onNavigate?})` — one-form "New chat" dialog (person + team mode) replacing the bare drive-file create at the chat sidebar/empty-state + contacts "Start chat". Open-don't-duplicate: `useFindChatByMembers(ownerId, emails)` (`chatKeys.byMembers`, emails lowercased+sorted) hits `GET /chat/:o/rooms/by-members` → `ChatMatch[]` (writable-first) so an exact effective-member match opens instead of creating; `useCreateChatRoom` posts `POST /chat/:o/:m/rooms` (server-side create + share in one step, share email suppressed); `useStartChatWith()` returns the contacts orchestrator `startChatWith(email)` (one writable match opens cross-app, else wizard pre-filled). Team mode creates via the generic `useCreateChat`. See [CHAT.md](docs/CHAT.md#new-chat-wizard) |
