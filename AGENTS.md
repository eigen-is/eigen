...
| **Upload pipeline**   | `apps/api/src/lib/sync/` + `Mount` `pending_uploads` | Write-behind S3 uploads (Phase 1b). `isRemote` mounts stage a frozen `VACUUM INTO` copy + enqueue in `metadata.db`, then a process-global semaphore + retry sweep drain it off the request path; failures back off + replay on restart/reopen. `local`/`local-key` stay synchronous. See [PROPOSAL_SYNC_RESILIENCE.md](docs/PROPOSAL_SYNC_RESILIENCE.md) |
