...
| **Document transforms** | `apps/api/src/lib/document/transform/`     | Bounded runner + one-shot Bun Workers for CPU-heavy document transforms (every eigensheets/eigendoc/eigenslides preview, every HTML/XLSX/PDF/DOCX export, and the xlsx + docx import/convert). One main-thread seam (`run-transform.ts`) that owns each operation's deadline, typed closed protocol, never a main-thread fallback; media prep and the import commit stay on the main thread. Same layout per type: Worker-pure modules (`export/<type>/{render,transform}.ts`, `preview/eigen<type>-render.ts`, `import/<type>/transform.ts`) behind thin main-thread wrappers (`export/export-document.ts`, `preview/eigen<type>-preview.ts`, `import/import-document.ts`). See [PREVIEWS.md](docs/PREVIEWS.md) + [EXPORT.md](docs/EXPORT.md) + [PROPOSAL_DOCUMENT_TRANSFORM_WORKERS.md](docs/PROPOSAL_DOCUMENT_TRANSFORM_WORKERS.md) |
