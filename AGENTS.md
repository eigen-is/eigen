...
| **Command palette**| `packages/lib/src/core/command-palette/` + `packages/ui/src/components/layout/app/command-palette/` | `Mod+K` dialog. Lib: engine + parsers + rank + providers + commands + hooks. UI: dialog + rows + trigger pill. Mounted by `AppShell.PaletteRunner`; gated via `useOptionalCommandPalette` so apps without the provider (index/blog/support) don't crash. See [PROPOSAL_COMMAND_PALETTE.md](docs/PROPOSAL_COMMAND_PALETTE.md) |
| **Search**         | `apps/api/src/lib/mail/db-config.ts` (FTS5 in mail.db v3) + `apps/api/src/lib/mail/maildb.ts` (`searchMail` JOIN) + `apps/api/src/routes/search.ts` + `packages/lib/src/core/search/` | Per-scope inline FTS5. Backend: `MailDB.searchMail` does a two-pass FTS5 JOIN + Drizzle hydrate, with filter-first via `mail.db` for `from`/`to`. Frontend: `useSearch` hook. SSE handlers invalidate `searchKeys.owner` on mail mutations. See [PROPOSAL_SEARCH.md](docs/PROPOSAL_SEARCH.md) |
...
`ToolbarTitle` component (`@workspace/ui/components/layout/toolbar`) — `text-sm text-foreground font-medium
